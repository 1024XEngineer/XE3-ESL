...
void main() {
  ...
  testWidgets('message confirmation opens the exact created practice', (
    ...
    final plan = _plan(sourceThreadId: harness.agent.threadId!);
    ...
    expect(harness.agent.practiceSessionId, _sessionId);
  ...
  test(
    ...
    () async {
      final harness = await _createHarness();
      addTearDown(harness.dispose);
      final sourceThreadId = harness.agent.threadId!;
      ...
      expect(harness.agent.threadId, harness.workspace.currentPracticeThreadId);
      expect(harness.agent.practiceSessionId, _sessionId);
  ...
  test('rejects a handoff that no longer matches the persisted plan', () async {
    ...
    expect(harness.agent.threads, hasLength(initialThreadCount));
...
Future<_Harness> _createHarness({AgentClient? client}) async {
  final practice = FakePracticeClient(
  ...
  return _Harness(agent: agent, workspace: workspace);
}
...
final class _Harness {
  const _Harness({required this.agent, required this.workspace});
...
final class _SeededHandoffAgentClient
