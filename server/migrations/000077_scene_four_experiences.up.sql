...
BEGIN
    IF EXISTS (
        ...
        WHERE scene_selection #>> '{scene,practice_experience}' = 'ROLEPLAY'
    ) OR EXISTS (
        SELECT 1 FROM practice_sessions
        WHERE practice_experience = 'ROLEPLAY'
    ) OR EXISTS (
        ...
        WHERE canonical_payload #>>
            '{practice_context,practice_experience}' = 'ROLEPLAY'
    ) OR EXISTS (
        SELECT 1 FROM evaluation_general_scene_results
        WHERE result_payload ->> 'practice_experience' = 'ROLEPLAY'
    ) OR EXISTS (
        SELECT 1 FROM evaluation_formal_reports
        WHERE practice_experience = 'ROLEPLAY'
    ) THEN
        RAISE EXCEPTION
            'ROLEPLAY practice data must be cleared before the four-Experience migration';
    END IF;
END;
$$;
...
SET practice_experience = CASE scene_category
        WHEN 'ROLEPLAY_WORKPLACE' THEN 'WORKPLACE'
        WHEN 'ROLEPLAY_TRAVEL' THEN 'LIFE_AND_TRAVEL'
        WHEN 'ROLEPLAY_DAILY' THEN 'LIFE_AND_TRAVEL'
    END,
    scene_category = CASE scene_category
        WHEN 'ROLEPLAY_WORKPLACE' THEN 'WORKPLACE_GENERAL'
        WHEN 'ROLEPLAY_TRAVEL' THEN 'LIFE_TRAVEL'
        WHEN 'ROLEPLAY_DAILY' THEN 'LIFE_DAILY'
    END
WHERE practice_experience = 'ROLEPLAY'
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM coaching_scene_versions
        WHERE practice_experience = 'ROLEPLAY'
           OR scene_category LIKE 'ROLEPLAY_%'
    ) THEN
        RAISE EXCEPTION 'unmapped ROLEPLAY Scene version';
    END IF;
END;
$$;
    ...
    IF jsonb_typeof(payload) IS DISTINCT FROM 'object'
       ...
       OR NOT (
            ...
            OR (
                ...
                AND practice_experience = 'WORKPLACE'
                AND scene_category = 'WORKPLACE_GENERAL'
                AND practice_mode IN ('FULL_SIMULATION', 'FOCUS')
            )
            OR (
                scene_type = 'OVERSEAS_DAILY_LIFE'
                AND practice_experience = 'LIFE_AND_TRAVEL'
                AND scene_category IN ('LIFE_TRAVEL', 'LIFE_DAILY')
                AND practice_mode IN ('FULL_SIMULATION', 'FOCUS')
            )
       )
