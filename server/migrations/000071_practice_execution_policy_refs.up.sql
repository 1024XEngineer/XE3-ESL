...
BEGIN
    IF EXISTS (SELECT 1 FROM preparation_practice_plans)
       OR EXISTS (SELECT 1 FROM practice_sessions) THEN
        RAISE EXCEPTION
            'Practice Execution Policy migration requires empty development and test Practice data. RECREATE THE DEVELOPMENT OR TEST DATABASE, then rerun migrations.'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
SET session_policy_ref = 'daily.practice.session.v1'
WHERE scene_id IN (
...
AND session_policy_ref = 'generic.practice.session.v1';

UPDATE coaching_scene_versions
SET session_policy_ref = 'workplace.practice.session.v1'
WHERE scene_id IN (
...
UPDATE coaching_scene_versions
SET session_policy_ref = 'interview.practice.session.v1'
WHERE scene_id IN (
...
UPDATE coaching_scene_versions
SET session_policy_ref = 'exam.practice.session.v1'
WHERE scene_id = 'scn_speaking_exam_custom'
  AND session_policy_ref = 'generic.practice.session.v1';
