...
BEGIN
    IF EXISTS (SELECT 1 FROM preparation_practice_plans)
       OR EXISTS (SELECT 1 FROM practice_sessions) THEN
        RAISE EXCEPTION
            'Scene Evaluation Policy migration requires empty development and test Practice data. RECREATE THE DEVELOPMENT OR TEST DATABASE, then rerun migrations.';
    END IF;
END;
$$;
...
SET evaluation_policy_ref = policy.evaluation_policy_ref
FROM (
    VALUES
        ('scn_programmer_interview', 'interview.shadow.evaluation.v1'),
        ('scn_interview_behavioral', 'interview.shadow.evaluation.v1'),
        ('scn_interview_custom', 'interview.shadow.evaluation.v1'),
        ('scn_interview_hiring_manager', 'interview.shadow.evaluation.v1'),
        ('scn_interview_recruiter_screening', 'interview.shadow.evaluation.v1'),
        ('scn_interview_self_introduction', 'interview.shadow.evaluation.v1'),
        ('scn_interview_system_design_spoken', 'interview.shadow.evaluation.v1'),
        ('scn_ielts_speaking_part_1', 'ielts.speaking_practice.evaluation.v1'),
        ('scn_ielts_speaking_part_2', 'ielts.speaking_practice.evaluation.v1'),
        ('scn_ielts_speaking_part_3', 'ielts.speaking_practice.evaluation.v1'),
        ('scn_ielts_speaking_full', 'ielts.speaking_full_mock.evaluation.v1'),
        ('scn_speaking_exam_custom', 'ielts.speaking_practice.evaluation.v1'),
        ('scn_workplace_client_delay', 'workplace.general.evaluation.v1'),
        ('scn_workplace_cross_team_alignment', 'workplace.general.evaluation.v1'),
        ('scn_workplace_custom', 'workplace.general.evaluation.v1'),
        ('scn_workplace_feedback_conflict', 'workplace.general.evaluation.v1'),
        ('scn_workplace_meeting_disagreement', 'workplace.general.evaluation.v1'),
        ('scn_workplace_negotiation', 'workplace.general.evaluation.v1'),
        ('scn_workplace_progress_risk_update', 'workplace.general.evaluation.v1'),
        ('scn_workplace_solution_presentation', 'workplace.general.evaluation.v1'),
        ('scn_daily_airport_transport', 'daily.general.evaluation.v1'),
        ('scn_daily_complaint_help', 'daily.general.evaluation.v1'),
        ('scn_daily_custom', 'daily.general.evaluation.v1'),
        ('scn_daily_hotel_checkin_issue', 'daily.general.evaluation.v1'),
        ('scn_daily_medical_appointment', 'daily.general.evaluation.v1'),
        ('scn_daily_phone_call', 'daily.general.evaluation.v1'),
        ('scn_daily_rental_maintenance', 'daily.general.evaluation.v1'),
        ('scn_daily_restaurant_ordering', 'daily.general.evaluation.v1'),
        ('scn_daily_shopping_return', 'daily.general.evaluation.v1'),
        ('scn_daily_small_talk', 'daily.general.evaluation.v1'),
        ('scn_daily_social_invitation', 'daily.general.evaluation.v1')
) AS policy(scene_id, evaluation_policy_ref)
WHERE version.scene_id = policy.scene_id;
...
BEGIN
    IF EXISTS (
        ...
        WHERE evaluation_policy_ref IS NULL
    ) THEN
        RAISE EXCEPTION
            'Scene versions without an explicit Evaluation Policy exist; recreate the development or test database before applying migration 000069'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
ALTER TABLE practice_sessions
    ADD COLUMN evaluation_policy_ref text COLLATE "C" NOT NULL,
    ADD CONSTRAINT practice_sessions_evaluation_policy_ref_check
        CHECK (
            char_length(evaluation_policy_ref) BETWEEN 15 AND 128
            AND evaluation_policy_ref = btrim(evaluation_policy_ref)
            AND evaluation_policy_ref ~ '^[a-z][a-z0-9._-]{0,127}$'
