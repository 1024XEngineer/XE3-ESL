...
func TestPostgresImageAssetRepositoryLifecycleAndIsolation(t *testing.T) {
...
INSERT INTO agent_messages (
	...
	repository, err := imagepostgres.New(
		database.pool,
	)
	if err != nil {
...
func TestPostgresImageSubmissionCommitsMessageRunAndImagesTogether(
	t *testing.T,
) {
	database := newAgentTestDatabase(t)
	ctx := context.Background()
...
VALUES ($1, $2)`,
	...
	imageRepository, err :=
		imagepostgres.New(database.pool)
	if err != nil {
	...
	ids := &imageSubmissionTestIDs{
		values: []string{imageTestMessageID, imageTestRunID},
	}
	repository, err :=
		runpostgres.NewImageSubmissionRepository(
			database.pool,
			ids,
		)
	if err != nil {
...
type imageSubmissionTestIDs struct {
	mu     sync.Mutex
	values []string
}

func (ids *imageSubmissionTestIDs) NewID() (string, error) {
	ids.mu.Lock()
	defer ids.mu.Unlock()
	if len(ids.values) == 0 {
