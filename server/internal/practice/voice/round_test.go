...
func TestVoiceTurnProgressRequiresAuthoritativeLimitCompletionEquivalence(
	...
	for name, progress := range map[string]TurnProgress{
	...
	if !validVoiceTurnProgress(TurnProgress{
		EffectiveTurns: 1,
		SessionVersion: 2,
		TurnLimit:      6,
	}) {
...
func TestNewVoiceRoundOrchestratorRequiresConversationPort(t *testing.T) {
	_, err := NewRoundOrchestrator(
		nil,
	...
	if err == nil {
		t.Fatal("NewRoundOrchestrator accepted a missing Conversation port")
	}
}
...
func TestVoiceRoundOrchestratorSubmitsTextThroughConversationPort(
	t *testing.T,
) {
	conversations := newAgentVoiceConversation(1)
	orchestrator := newAgentVoiceOrchestrator(
	...
	turn, err := orchestrator.SubmitText(
		context.Background(),
		agentVoiceActor("a"),
	...
	if turn.AnswerText != "Typed answer." || turn.EffectiveTurns != 1 ||
...
func (port *agentVoiceConversation) SubmitTextAnswer(
	...
	for id, candidate := range port.candidates {
		if candidate.SessionID == command.SessionID &&
			candidate.QuestionID == command.QuestionID {
			candidate.Transcript = command.AnswerText
			candidate.Provider = "user-text"
			candidate.Model = "direct-input"
			port.candidates[id] = candidate
...
func newAgentVoicePracticeWithLimit(
	...
	return &agentVoicePractice{
		turns:          make(map[string]TurnProgress),
...
func (practice *agentVoicePractice) ApplyEffectiveTurn(
	...
	if actor.UserID != "user-a" || sessionID != "session-1" {
		return TurnProgress{}, conversation.ErrVoiceRoundNotFound
	}
...
type agentVoiceReview struct {
	mu              sync.Mutex
	bySession       map[string]ReviewCheckpoint
	creations       int
...
func newAgentVoiceReview() *agentVoiceReview {
	return &agentVoiceReview{
		bySession: make(map[string]ReviewCheckpoint),
	}
}
...
func (reviews *agentVoiceReview) EnsureSessionReview(
	...
	if actor.UserID != "user-a" ||
		source.TurnID == "" ||
		source.SessionID == "" {
		return ReviewCheckpoint{}, conversation.ErrVoiceRoundNotFound
	}
	...
	if reviews.failAfterCreate {
		...
		return ReviewCheckpoint{}, errAgentVoiceLostAcknowledgement
	}
	return result, nil
}
...
type agentVoiceCompletionEvaluation struct {
	mu              sync.Mutex
	bySession       map[string]CompletionEvaluationSource
	calls           int
	creations       int
	failAfterCreate bool
}
...
func newAgentVoiceCompletionEvaluation() *agentVoiceCompletionEvaluation {
	return &agentVoiceCompletionEvaluation{
		bySession: make(map[string]CompletionEvaluationSource),
	}
}
...
func (evaluations *agentVoiceCompletionEvaluation) EnsureCompletedSessionEvaluation(
	_ context.Context,
	actor requestcontext.Actor,
	source CompletionEvaluationSource,
) error {
	evaluations.mu.Lock()
...
var (
	_ ConversationPort = (*agentVoiceConversation)(nil)
	_ ConversationPort = (*conversation.VoiceRoundService)(nil)
)
