...
func (handler *Handler) RegisterRoutes(routes gin.IRoutes) {
	...
	if handler.audioAssets != nil {
		_ = practiceconversation.RegisterAudioAssetRoutes(
			routes,
			handler.audioAssets,
			practiceconversation.AudioAssetActorResolverFunc(
				func(request *http.Request) (
					practiceconversation.AudioAssetActor,
					bool,
				) {
					actor, ok := requestcontext.ActorFromContext(request.Context())
					return practiceconversation.AudioAssetActor{UserID: actor.UserID},
						ok && actor.Valid()
				},
			),
		)
...
func (handler *Handler) transcribeCandidate(c *gin.Context) {
	...
	candidate, err := handler.application.Transcribe(
		c.Request.Context(),
		actor,
		practiceconversation.TranscribeVoiceCommand{
...
func (handler *Handler) confirmCandidate(c *gin.Context) {
	...
	state, err := handler.application.Confirm(
		c.Request.Context(),
		actor,
		practiceconversation.ConfirmVoiceTurnCommand{
...
func (handler *Handler) submitText(c *gin.Context) {
	...
	state, err := handler.application.SubmitText(
		...
		practiceconversation.SubmitTextAnswerCommand{
			SessionID:      c.Param("practice_session_id"),
			QuestionID:     c.Param("question_id"),
...
func QuestionResponse(question practiceconversation.VoiceQuestion) gin.H {
	result := gin.H{
		...
		"question_type":       question.Type, "content": question.Text,
...
func TranscriptionCandidateResponse(
	candidate practiceconversation.TranscriptionCandidate,
) gin.H {
	return gin.H{
...
func ConfirmedTurnResponse(turn practiceconversation.ConfirmedVoiceTurn) gin.H {
	result := gin.H{
...
func SessionReviewResponse(item practicevoice.SessionReview) gin.H {
	result := gin.H{
		"review_id": item.ID, "practice_session_id": item.SessionID,
		"status":                 item.Status,
...
func mapError(err error) error {
	...
	case errors.Is(err, practicevoice.ErrInvalidRequest),
		...
		errors.Is(err, practiceconversation.ErrVoiceRoundInvalid):
		return invalidRequest(err)
	case errors.Is(err, practicevoice.ErrNotFound),
		errors.Is(err, practiceconversation.ErrVoiceRoundNotFound):
		return resourceNotFound(err)
	case errors.Is(err, practicevoice.ErrIdempotencyConflict),
		errors.Is(err, practiceconversation.ErrVoiceRoundConflict):
		return apperror.New(
	...
	case errors.Is(err, practiceconversation.ErrVoiceRoundProcessing):
		return apperror.New(
	...
	case errors.Is(err, practiceconversation.ErrVoiceRoundCapacity):
		return apperror.New(
