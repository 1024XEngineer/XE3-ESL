...
func validInterviewParticipants(sessionID string, participants []PracticeParticipantInput) bool {
	...
	return participant.ID != "" && participant.SessionID == sessionID && participant.ParticipantRole == ParticipantRoleInterviewer &&
		participant.SubjectRef.Namespace != "" && participant.SubjectRef.SubjectID != "" && participant.ParticipantOrder > 0 &&
		participant.RoleDefinitionID != "" && participant.RoleDefinitionID == participant.RoleSnapshot.RoleDefinitionID && participant.RoleSnapshot.Version > 0
}
