...
func TestContextRepositoryRequiresCurrentReadyPlanRevision(t *testing.T) {
	...
	created, replayed, err := repository.CreateContextSession(
		context.Background(),
		owner.Actor,
		current,
	)
	if err != nil || replayed || created.Session.PlanRevision != revised.Revision {
		t.Fatalf("CreateContextSession current = (%#v,%t,%v)", created, replayed, err)
	}
	recovered, err := practicepostgres.New(pool).ResolveContextSessionByPlan(
		context.Background(),
		owner.Actor,
		plan.ID,
	)
	if err != nil || recovered.Session.ID != created.Session.ID ||
		...
		t.Fatalf("ResolveContextSessionByPlan = (%#v,%v)", recovered, err)
	}
}
...
func TestContextRepositoryRejectsArchivedPlanAndConflictsByPlan(t *testing.T) {
	...
	if _, _, err := repository.CreateContextSession(context.Background(), owner.Actor, first); err != nil {
	...
	if _, _, err := repository.CreateContextSession(
		...
		owner.Actor,
		second,
	); !errors.Is(err, persistence.ErrActiveSessionConflict) {
...
func contextSessionCommand(
	...
	for _, role := range roles {
		...
		participants = append(participants, persistence.ContextParticipant{
			...
			Role:             "FACILITATOR",
			SubjectRef:       persistence.SubjectRef{Namespace: "speakup.role", SubjectID: role.ID},
	...
	participants = append(participants, persistence.ContextParticipant{
		...
		SubjectRef: persistence.SubjectRef{Namespace: "speakup.user", SubjectID: owner.Actor.UserID},
		Order:      len(participants) + 1,
	})
...
func contextActor(actor persistence.Actor) requestcontext.Actor {
