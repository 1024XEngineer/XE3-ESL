...
func (a *ContextApplication) ConfirmAndStartPractice(
	...
	if errors.Is(err, persistence.ErrActiveSessionConflict) {
		active, resolveErr := a.repository.ResolveContextSessionByPlan(
			ctx,
			contextActor(actor),
			confirmation.PracticePlanID,
		)
...
func (a *ContextApplication) ResolveSessionByPlan(
	...
	return a.repository.ResolveContextSessionByPlan(
		ctx,
		contextActor(actor),
		planID,
	)
...
func contextActor(actor requestcontext.Actor) persistence.Actor {
	return persistence.Actor{UserID: actor.UserID, SessionID: actor.SessionID}
}
