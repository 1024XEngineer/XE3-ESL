...
func ensureSpeechFeedbackTurnSnapshot(
	...
	snapshotID := "review_snapshot_" +
		hex.EncodeToString(digest[:16])
	evidenceRefID := "review_evidence_" +
		hex.EncodeToString(digest[16:])
	if _, err := tx.Exec(ctx, `
		INSERT INTO review_speech_feedback_turn_snapshots (
			id,
			owner_user_id,
			practice_session_id,
			turn_id,
...
const speechFeedbackSelect = `
	...
	LEFT JOIN review_speech_feedback_turn_snapshots AS snapshot
...
func listSpeechFeedbackItems(
	...
	rows, err := database.Query(ctx, `
		SELECT
			...
			repractice_mode,
			created_at
		FROM review_speech_feedback_items
		WHERE owner_user_id = $1
