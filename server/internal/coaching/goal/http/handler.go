...
func NewHandler(
	...
	if application == nil {
		return nil, errors.New("goal: HTTP application is required")
	}
...
func (handler *Handler) RegisterRoutes(routes gin.IRoutes) {
	routes.POST("/v1/goals", handler.create)
	routes.GET("/v1/goals", handler.list)
	routes.GET("/v1/goals/:goal_id", handler.get)
	routes.PATCH("/v1/goals/:goal_id", handler.changeStatus)
}
...
func Response(item goal.Goal) gin.H {
	return gin.H{
		"goal_id":    item.ID,
		"title":      item.Title,
		"status":     item.Status,
...
func mapError(err error) error {
	...
	case errors.Is(err, goal.ErrConflict):
		return apperror.New(
			apperror.Conflict,
			"resource_conflict",
