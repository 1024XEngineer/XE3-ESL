		...
		t.Run(test.name, func(t *testing.T) {
			...
			if report, exists := body["report"].(map[string]any); exists {
				if report["schema_version"] !=
					evaluationreport.InterviewReportSchemaVersion {
...
func insufficientInterviewReport() evaluationreport.InterviewReport {
	dimensions := make(
		[]evaluationreport.InterviewReportDimension,
		0,
		5,
	)
	dimensionFindings := make(
		[]evaluationreport.InterviewQuestionDimensionRefs,
		0,
		5,
	)
	for _, dimensionID := range scoring.InterviewDimensions() {
		dimensions = append(
			...
			evaluationreport.InterviewReportDimension{
				...
				ScoreabilityStatus: scoring.InterviewScoreabilityInsufficient,
				GateStatus:         scoring.InterviewGateBlocked,
				ReasonCodes: []scoring.InterviewReasonCode{
					scoring.InterviewReasonInsufficientEvidence,
				},
				EvidenceRefIDs:         []string{},
				Strengths:              []evaluationreport.InterviewReportFinding{},
				Improvements:           []evaluationreport.InterviewReportFinding{},
				RecommendedExpressions: []evaluationreport.InterviewReportFinding{},
			},
		)
	...
	return evaluationreport.InterviewReport{
		SchemaVersion:      evaluationreport.InterviewReportSchemaVersion,
		ScoreabilityStatus: scoring.InterviewScoreabilityInsufficient,
		GateStatus:         scoring.InterviewGateBlocked,
		ReadinessLevel:     scoring.InterviewReadinessNotAssessed,
		ReadinessNotice:    evaluationreport.InterviewReportReadinessNotice,
		Dimensions:         dimensions,
		Questions: []evaluationreport.InterviewReportQuestion{{
			QuestionID:        "question-1",
			QuestionType:      "PRIMARY",
			OpportunityStatus: scoring.InterviewOpportunityNotProvided,
			AssessmentStatus:  evaluationreport.InterviewAssessmentNotAssessed,
			QuestionText:      "Tell me about a migration you led.",
			EvidenceRefIDs:    []string{},
			DimensionFindings: dimensionFindings,
		}},
		PriorityActions: []evaluationreport.InterviewReportPriorityRef{},
	}
}
