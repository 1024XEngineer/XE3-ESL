...
func TestInterviewShadowRejectsWrongSnapshotBoundary(t *testing.T) {
	...
	tests := []evidence.EvidenceSnapshot{
		func() evidence.EvidenceSnapshot {
			value := snapshot
			value.SceneType = evaluation.SceneOverseasDaily
			return value
		}(),
		func() evidence.EvidenceSnapshot {
			value := snapshot
...
func interviewShadowTestSnapshot(
	...
	if err != nil {
		t.Fatalf("canonicalize provisional evidence.EvidenceSnapshot: %v", err)
	}
	sourceManifestHash, err := evidence.SourceManifestHash(provisional)
	if err != nil {
		t.Fatalf("derive evidence.EvidenceSnapshot source manifest: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("canonicalize evidence.EvidenceSnapshot: %v", err)
	}
	...
	if !snapshot.Valid() {
		t.Fatal("Interview evidence.EvidenceSnapshot fixture is invalid")
	}
	return snapshot
}
