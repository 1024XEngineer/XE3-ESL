...
const LearningProfileStrategyVersion = "learning-profile-weighted-mean/v1"

type LearningProfileTrend string

const (
	LearningProfileTrendInitial   LearningProfileTrend = "INITIAL"
	LearningProfileTrendStable    LearningProfileTrend = "STABLE"
	LearningProfileTrendImproving LearningProfileTrend = "IMPROVING"
	LearningProfileTrendDeclining LearningProfileTrend = "DECLINING"
)
...
type LearningProfileSourceRef struct {
...
type LearningProfileDimension struct {
	...
	Scale             ReportScoreScale           `json:"score_scale"`
	EstimatedValue    float64                    `json:"estimated_value"`
	Confidence        float64                    `json:"confidence"`
	Trend             LearningProfileTrend       `json:"trend"`
	RecurringIssues   []LearningProfileIssue     `json:"recurring_issues"`
	SourceEvaluations []LearningProfileSourceRef `json:"source_evaluations"`
...
type LearningProfileQuery struct {
	GoalID    string
	SceneType SceneType
	Limit     int
}
...
func persistFormalReportAndLearningProfile(
	...
	if ctx == nil || tx == nil || !report.Valid() ||
		report.SceneType != claim.Snapshot.SceneType {
	...
	`, claim.EvaluationID, claim.EvaluationRevisionID,
		...
		claim.Revision, report.SceneType, report.SceneModel,
		report.ScoreabilityStatus, report.SchemaVersion, encoded).Scan(
		&reportID,
	)
	...
	if report.ScoreabilityStatus != ReportScoreabilityProvisional {
		return nil
	}
	projected := make([]string, 0, len(report.Dimensions))
	for _, dimension := range report.Dimensions {
		...
		`, claim.OwnerUserID, claim.EvaluationID,
			...
			*dimension.Score, dimension.Confidence, issueJSON,
			dimension.EvidenceRefs, LearningProfileStrategyVersion)
		if err != nil {
...
func rebuildLearningProfileDimension(
	...
	rows, err := tx.Query(ctx, `
		...
		LIMIT 20
	`, ownerUserID, dimensionKey, LearningProfileStrategyVersion)
	if err != nil {
	...
	`, ownerUserID, dimension.Key, dimension.Scale,
		...
		LearningProfileStrategyVersion)
	if err != nil {
...
func aggregateLearningProfileDimension(
	...
	if !validVersion(key) || len(contributions) == 0 || len(contributions) > 20 {
		return LearningProfileDimension{}, ErrInvalidRequest
	}
	scale := contributions[0].Scale
	weightedScore := 0.0
	weightTotal := 0.0
	confidenceTotal := 0.0
	issues := make(map[string]LearningProfileIssue)
	sources := make([]LearningProfileSourceRef, 0, len(contributions))
	for _, contribution := range contributions {
		...
		sources = append(sources, LearningProfileSourceRef{
	...
	recurring := make([]LearningProfileIssue, 0, len(issues))
	for _, issue := range issues {
		recurring = append(recurring, issue)
	}
	slices.SortFunc(recurring, func(left, right LearningProfileIssue) int {
	...
	return LearningProfileDimension{
		Key:               key,
		Scale:             scale,
		...
		StrategyVersion:   LearningProfileStrategyVersion,
...
func validLearningProfileTrend(trend LearningProfileTrend) bool {
	switch trend {
	case LearningProfileTrendInitial,
		LearningProfileTrendStable,
		LearningProfileTrendImproving,
		LearningProfileTrendDeclining:
		return true
	default:
		return false
	}
...
func learningProfileTrend(
	scale ReportScoreScale,
	contributions []learningProfileContribution,
) LearningProfileTrend {
	if len(contributions) < 2 {
		return LearningProfileTrendInitial
	}
	delta := contributions[0].Score - contributions[1].Score
	threshold := 2.0
	if scale == ReportScaleIELTSBand {
		threshold = 0.5
	}
	switch {
	case delta >= threshold:
		return LearningProfileTrendImproving
	case delta <= -threshold:
		return LearningProfileTrendDeclining
	default:
		return LearningProfileTrendStable
	}
}
