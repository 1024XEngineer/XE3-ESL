...
const speechFeedbackRepairSystemPrompt = speechFeedbackSystemPrompt + ` The input includes normalize_reason, a finite validation category for one rejected response. Generate a new response from kind and english_text that corrects that category. The rejected response is not included and must not be inferred. Return only the newly generated JSON object.`
