...
func TestCanonicalEvidencePayloadIsStableAndRejectsStorageLocators(
	...
	firstCanonical, err := canonicalEvidencePayload(first)
	if err != nil {
		t.Fatalf("canonicalize first payload: %v", err)
	}
	secondCanonical, err := canonicalEvidencePayload(second)
	if err != nil {
	...
	for _, invalid := range []json.RawMessage{
		...
		if _, err := canonicalEvidencePayload(invalid); !errors.Is(
			err,
			ErrInvalidRequest,
		) {
...
func TestEvidenceSnapshotValidVerifiesCanonicalPayloadHash(t *testing.T) {
	sourceManifestHash, err := evidenceSourceManifestHash(
...
func TestEvidenceSourceManifestBindsCompleteConfirmedTurn(t *testing.T) {
	...
	original, err := evidenceSourceManifestHash(payload)
	if err != nil {
	...
	changed, err := evidenceSourceManifestHash(changedPayload)
	if err != nil {
