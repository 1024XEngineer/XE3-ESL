...
func (r *EvidenceSourceReader) Compose(
	...
	if r == nil || r.practice == nil || r.voice == nil ||
		...
		scope != evaluation.ScopeSession || !validSceneType(sceneType) {
...
func (r *EvidenceSourceReader) ComposeCompleted(
	...
	if r == nil || r.practice == nil || r.voice == nil ||
		...
		!validIdentifier(practiceSessionID) || scope != evaluation.ScopeSession ||
...
func (r *EvidenceSourceReader) compose(
	...
	payload := SnapshotPayload{
		...
		ConfirmedTurns: make([]ConfirmedTurn, 0, len(turns)),
		EvidenceRefs:   make([]Ref, 0, len(turns)),
		ProviderLineage: ProviderLineage{
			ASR: make([]ASRLineage, 0, len(turns)),
		...
		VersionManifest: VersionManifest{
			...
			TurnEvidence:          make([]TurnVersion, 0, len(turns)),
...
func (r *EvidenceSourceReader) composeTurn(
	...
	if turn.SessionID != practiceSessionID ||
		...
		return composedEvidenceTurn{}, evaluation.ErrInvalidRequest
	}
...
func (r *EvidenceSourceReader) readEvidenceAudio(
	...
	if !validEvidenceAudio(asset, ownerUserID, turn) {
		return Audio{}, 0, evaluation.ErrInvalidRequest
	}
...
type SnapshotPayload struct {
	PracticeContext     PracticeContext `json:"practice_context"`
	OpportunityManifest []Opportunity   `json:"opportunity_manifest"`
	ConfirmedTurns      []ConfirmedTurn `json:"confirmed_turns"`
	EvidenceRefs        []Ref           `json:"evidence_refs"`
	ProviderLineage     ProviderLineage `json:"provider_lineage"`
	VersionManifest     VersionManifest `json:"version_manifest"`
}
...
type PreparationContext struct {
	...
	JobTargetInput                     *JobTargetInput     `json:"job_target_input,omitempty"`
	JobTargetCandidate                 *JobTargetCandidate `json:"job_target_candidate,omitempty"`
...
type JobTargetCandidate struct {
	...
	CatalogRecommendation CatalogRecommendation `json:"catalog_recommendation"`
}

type CatalogRecommendation struct {
...
type Ref struct {
	...
	TranscriptSpan TranscriptSpan `json:"transcript_span"`
	AudioSpan      *AudioSpan     `json:"audio_span,omitempty"`
	Quality        Quality        `json:"quality"`
	Lineage        RefLineage     `json:"lineage"`
}
...
type ProviderLineage struct {
	ASR                  []ASRLineage         `json:"asr"`
	UnavailableArtifacts UnavailableArtifacts `json:"unavailable_artifacts"`
}

type ASRLineage struct {
	TurnID            string `json:"turn_id"`
	TranscriptID      string `json:"transcript_id"`
	CandidateID       string `json:"candidate_id"`
	EvidenceVersion   int64  `json:"evidence_version"`
...
type VersionManifest struct {
	...
	PlanRevision          int           `json:"plan_revision"`
	TurnEvidence          []TurnVersion `json:"turn_evidence"`
	AudioQuality          string        `json:"audio_quality"`
	ISE                   string        `json:"ise"`
	FeatureBundle         string        `json:"feature_bundle"`
...
func evidenceParticipants(
	...
	result := make([]Participant, 0, len(source))
	...
	slices.SortFunc(result, func(left, right Participant) int {
...
func evidenceObjectives(
	...
	slices.SortFunc(result, func(left, right Objective) int {
		return strings.Compare(left.ID, right.ID)
	})
	return result
}
...
func evidencePreparationContextFromSnapshot(
	...
	if input := source.JobTargetInputSnapshot; input != nil {
		result.JobTargetInput = &JobTargetInput{
	...
	if candidate := source.JobTargetCandidateSnapshot; candidate != nil {
		result.JobTargetCandidate = &JobTargetCandidate{
...
func CanonicalJSON(payload SnapshotPayload) (json.RawMessage, error) {
...
func evidenceSourceManifestFromPayload(
	...
	if len(payload.OpportunityManifest) < len(payload.ConfirmedTurns) ||
		...
		return evidenceSourceManifest{}, evaluation.ErrInvalidRequest
	}
	refsByTurn := make(map[string]Ref, len(payload.EvidenceRefs))
...
func SourceManifestHash(
	...
	if err := decoder.Decode(&evidence); err != nil {
		return [sha256.Size]byte{}, evaluation.ErrInvalidRequest
	}
