...
func TestPostgresSpeechFeedbackRejectsTextTurnWithoutReadableAudio(
	...
	repository := evaluation.NewPostgresRepository(pool)
	if _, err := repository.EnsureConfirmedConversationTurn(
		ctx,
		ownerID,
		sessionID,
		turnID,
	); !errors.Is(err, evaluation.ErrSpeechFeedbackNotApplicable) {
...
func TestPostgresSpeechFeedbackPersistsTopicAcousticEvidence(t *testing.T) {
	...
	err = repository.SaveSpeechFeedbackAcousticEvidence(
		context.Background(),
		claim,
		evaluation.SpeechFeedbackAcousticEvidence{
			Assessment: evaluation.SpeechFeedbackAcousticAssessment{
				Pronunciation:      evaluation.SpeechFeedbackAssessed,
				AcousticFluency:    evaluation.SpeechFeedbackAssessed,
...
func TestPostgresSpeechFeedbackLeaseFencingAndExactItemRead(
	...
	drafts := []evaluation.SpeechFeedbackDraftItem{{
		Kind: evaluation.SpeechFeedbackItemImprovement,
		Anchor: evaluation.SpeechFeedbackAnchor{
			AnchorKind:      evaluation.SpeechFeedbackAnchorConversationTranscript,
		...
		RepracticeMode: evaluation.SpeechFeedbackRepracticeSameQuestion,
	}}
	if _, err := repository.CompleteSpeechFeedback(
		context.Background(),
		first,
		drafts,
	); !errors.Is(err, evaluation.ErrSpeechFeedbackClaimLost) {
	...
	if completed.FeedbackStatus != evaluation.SpeechFeedbackReady ||
	...
	if _, err := repository.GetSpeechFeedback(
		...
		otherOwner,
		reference.SpeechFeedbackID,
	); !errors.Is(err, evaluation.ErrSpeechFeedbackNotFound) {
...
func TestPostgresSpeechFeedbackPersistsTerminalRetryableFailure(
	...
	status, err := repository.FailSpeechFeedback(
		context.Background(),
		claim,
		evaluation.SpeechFeedbackStableFailure{
			ReasonCode: evaluation.SpeechFeedbackFailureProviderUnavailable,
			Retryable:  true,
		},
		configuration,
	)
	...
	if status != evaluation.SpeechFeedbackFailed {
	...
	if persistedCode != string(
		evaluation.SpeechFeedbackFailureProviderUnavailable,
...
func TestPostgresSpeechFeedbackDeletionFencesLateWorker(
	...
	_, err = repository.CompleteSpeechFeedback(
		...
		[]evaluation.SpeechFeedbackDraftItem{{
			Kind: evaluation.SpeechFeedbackItemCorrection,
			Anchor: evaluation.SpeechFeedbackAnchor{
				AnchorKind:      evaluation.SpeechFeedbackAnchorConversationTranscript,
	...
	if !errors.Is(err, evaluation.ErrSpeechFeedbackClaimLost) {
...
func TestPostgresSpeechFeedbackUsesAgentTranscriptIdentityWithoutPracticeIDs(
	...
	if feedback.Source.SourceKind !=
		evaluation.SpeechFeedbackSourceAgentVoiceMessage ||
