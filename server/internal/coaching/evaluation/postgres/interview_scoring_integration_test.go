...
func TestPostgresInterviewShadowConcurrentClaimHasSingleWinner(
	...
	claims := make(chan scoring.InterviewShadowClaim, callers)
	...
	var winners []scoring.InterviewShadowClaim
...
func TestPostgresInterviewShadowClaimAndReevaluateDoNotDeadlock(
	...
	reevaluateDone := make(chan struct {
		evaluation evaluationcore.Evaluation
		err        error
	}, 1)
	go func() {
		<-start
...
func TestPostgresInterviewShadowCompleteAndReevaluateDoNotDeadlock(
	...
	if completeErr != nil &&
		!errors.Is(completeErr, scoring.ErrRuntimeLeaseLost) {
...
func TestPostgresInterviewShadowFailureBackoffAndExhaustion(
	...
	status, err := repository.FailInterviewShadow(
		context.Background(),
		first,
		scoring.InterviewShadowFailure{
	...
	if status != scoring.InterviewShadowRuntimePending {
	...
	if status != scoring.InterviewShadowRuntimeFailed {
...
) evaluationcore.ReevaluateCommand {
	return evaluationcore.ReevaluateCommand{
		...
		Config: evaluationcore.RevisionConfig{
			...
			SchemaVersion:    evaluationcore.SchemaVersion,
			ClientRequestID:  fingerprintSeed,
		},
	}
}
