...
func TestPostgresInterviewReportLookupIsOwnerScopedAndFailClosed(
	...
	if _, err := repository.GetCurrentInterviewReportState(
		...
		testOwnerA,
		value.PracticeSessionID,
	); !errors.Is(err, report.ErrInterviewConfigurationConflict) {
...
func TestPostgresInterviewReportReadsEveryPublishedState(t *testing.T) {
	...
	t.Run("ready", func(t *testing.T) {
		...
		report, err := report.ProjectInterviewReport(
			*state.Snapshot,
	...
	t.Run("failed", func(t *testing.T) {
		...
		if err != nil || status != scoring.InterviewShadowRuntimeFailed {
...
func insertDuplicateInterviewReportEvaluation(
	t *testing.T,
	pool *pgxpool.Pool,
	source evaluationcore.Evaluation,
) {
	t.Helper()
	...
	`, evaluationID, source.OwnerUserID,
		...
		evaluationcore.SchemaVersion).Scan(&revisionID); err != nil {
