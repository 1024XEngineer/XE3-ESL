...
func (r *Repository) SaveQuestion(
	...
	if !validInputActor(actor) || !validQuestion(question) {
		return practice.Question{}, practicevoice.ErrPersistenceInvalid
	}
	...
	if tag.RowsAffected() == 0 &&
		...
		return practice.Question{}, practicevoice.ErrPersistenceConflict
	}
...
func (r *Repository) ListSessionQuestions(
	...
	if !validInputActor(actor) || strings.TrimSpace(sessionID) == "" {
		return nil, practicevoice.ErrPersistenceInvalid
	}
...
func (r *Repository) ReserveTranscription(
	ctx context.Context,
	actor practicevoice.Actor,
	command practicevoice.StoreReserveTranscriptionCommand,
) (practicevoice.StoredTranscriptionReservation, error) {
	if !validInputActor(actor) ||
		...
		return practicevoice.StoredTranscriptionReservation{}, practicevoice.ErrPersistenceInvalid
	}

	tx, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return practicevoice.StoredTranscriptionReservation{}, safeDatabaseError(err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	deletionGeneration, err := ensureActorWritable(ctx, tx, actor.UserID)
	if err != nil {
		return practicevoice.StoredTranscriptionReservation{}, err
	}
	r.reachedWriteFence()
	if err := lockKey(
		ctx,
	...
	if question.SessionID != command.SessionID {
		return practicevoice.StoredTranscriptionReservation{}, practicevoice.ErrPersistenceNotFound
	}
	...
	if found {
		if reservation.InputFingerprint != command.InputFingerprint ||
			...
			return practicevoice.StoredTranscriptionReservation{}, practicevoice.ErrPersistenceConflict
		}
		if reservation.Status == practicevoice.StoredTranscriptionCompleted ||
			(reservation.Status == practicevoice.StoredTranscriptionProcessing &&
	...
	reservation = practicevoice.StoredTranscriptionReservation{
		...
		Status:                  practicevoice.StoredTranscriptionProcessing,
		FencingToken:            1,
...
func (r *Repository) CompleteTranscription(
	ctx context.Context,
	job practicevoice.JobContext,
	command practicevoice.StoreCompleteTranscriptionCommand,
	...
	if !validJob(job) ||
		...
		return practicevoice.StoredTranscriptCandidate{}, practicevoice.ErrPersistenceInvalid
	}

	tx, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return practicevoice.StoredTranscriptCandidate{}, safeDatabaseError(err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	if err := ensureJobWritable(ctx, tx, job); err != nil {
		return practicevoice.StoredTranscriptCandidate{}, err
	}
	r.reachedWriteFence()
	...
	if reservation.Status == practicevoice.StoredTranscriptionCompleted {
		...
		if candidateErr != nil {
			return practicevoice.StoredTranscriptCandidate{}, candidateErr
		}
		if reservation.FencingToken != job.FencingToken ||
			...
			return practicevoice.StoredTranscriptCandidate{}, practicevoice.ErrPersistenceConflict
		}
		if err := tx.Commit(ctx); err != nil {
			return practicevoice.StoredTranscriptCandidate{}, safeDatabaseError(err)
		}
	...
	if reservation.Status != practicevoice.StoredTranscriptionProcessing ||
	...
	candidate := practicevoice.StoredTranscriptCandidate{
		...
		Text:                    command.Text,
		Status:                  practicevoice.CandidateReady,
		CreatedAt:               now,
	}
	_, err = tx.Exec(
		ctx,
...
func (r *Repository) FailTranscription(
	ctx context.Context,
	job practicevoice.JobContext,
	failure practicevoice.ProcessingFailure,
) error {
	if !validJob(job) ||
...
func (r *Repository) GetCompletedCandidate(
	...
	if !validUUID(ownerUserID) || strings.TrimSpace(candidateID) == "" {
		return practicevoice.StoredTranscriptCandidate{},
			practicevoice.ErrPersistenceInvalid
	}
...
func (r *Repository) ListProcessingAttempts(
	...
	actor practicevoice.Actor,
	reservationID string,
) ([]practicevoice.ProcessingAttempt, error) {
	if !validInputActor(actor) || strings.TrimSpace(reservationID) == "" {
		return nil, practicevoice.ErrPersistenceInvalid
	}
	tx, err := r.beginActorRead(ctx, actor)
	...
	defer rows.Close()
	attempts := make([]practicevoice.ProcessingAttempt, 0)
	for rows.Next() {
		var attempt practicevoice.ProcessingAttempt
...
func (r *Repository) advanceConfirmedTurnInTransaction(
	...
	if turn.Kind == practice.TurnKindRetry {
		...
		if turn.EffectiveTurns < 1 {
			return practice.Turn{}, practicevoice.ErrPersistenceConflict
		}
...
func (r *Repository) confirmTurnInTransaction(
	...
	if command.RetryTurnID != "" {
		...
		if persistedDraft.PracticeSessionID != candidate.SessionID ||
			...
			return practice.Turn{},
				practicevoice.ErrPersistenceConflict
		}
...
func scanQuestion(row rowScanner) (practice.Question, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return practice.Question{}, practicevoice.ErrPersistenceNotFound
	}
	if err != nil {
		return practice.Question{}, safeDatabaseError(err)
	}
...
func findReservationByKey(
	...
	ownerUserID string,
	key string,
) (practicevoice.StoredTranscriptionReservation, bool, error) {
	...
	if errors.Is(err, practicevoice.ErrPersistenceNotFound) {
		return practicevoice.StoredTranscriptionReservation{}, false, nil
	}
...
func scanReservation(
	...
	var reservation practicevoice.StoredTranscriptionReservation
	err := row.Scan(
		&reservation.ID,
		&reservation.QuestionID,
		&reservation.SessionID,
...
func candidateQuery(
	...
	var candidate practicevoice.StoredTranscriptCandidate
	err := db.QueryRow(
		ctx,
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return practicevoice.StoredTranscriptCandidate{}, practicevoice.ErrPersistenceNotFound
	}
	if err != nil {
		return practicevoice.StoredTranscriptCandidate{}, safeDatabaseError(err)
	}
...
func ensureActorWritable(
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return 0, practicevoice.ErrPersistenceNotFound
	}
	if err != nil {
		return 0, safeDatabaseError(err)
	}
	if accountStatus != "active" || deletionGeneration != 0 {
		return 0, practicevoice.ErrActorDeleted
	}
...
func validInputActor(actor practicevoice.Actor) bool {
	return actor.Valid() && validUUID(actor.UserID)
}

func validJob(job practicevoice.JobContext) bool {
...
func confirmationHash(command practicevoice.ConfirmTurnCommand) string {
	payload := fmt.Sprintf(
		"%s\x00%d\x00%s",
		command.CandidateID,
...
func safeDatabaseError(err error) error {
	...
	return practicevoice.ErrPersistenceUnavailable
}

var _ practicevoice.PersistenceStore = (*Repository)(nil)
