...
func TestNewAudioAssetRepositoryRejectsNilPool(t *testing.T) {
	...
	if _, err := NewAudioAssetRepository(nil); !errors.Is(
		err,
		practicevoice.ErrAudioAssetInvalidDependency,
	) {
...
func TestAudioAssetRepositoryRejectsPaddedOpaqueIdentifiers(t *testing.T) {
	...
	if persisted.Status != practicevoice.AudioAssetStaged ||
		persisted.Version != 1 {
...
func TestAudioAssetRepositoryPersistsLifecycleAndOwnerScope(
	...
	if recovered.ID != alice.ID ||
		...
		recovered.Status != practicevoice.AudioAssetStaged ||
	...
	if _, err := restarted.GetOwned(
		...
		audioTestOwnerB,
		alice.ID,
	); !errors.Is(err, practicevoice.ErrAudioAssetNotFound) {
	...
	if err := repository.Create(ctx, duplicateRequest); !errors.Is(
		err,
		practicevoice.ErrAudioAssetConcurrentUpdate,
	) {
	...
	alice.Status = practicevoice.AudioAssetMetadataCommitted
	...
	alice.Status = practicevoice.AudioAssetReadable
	...
	conflictingBinding.Status = practicevoice.AudioAssetMetadataCommitted
	...
	conflictingBinding.Status = practicevoice.AudioAssetReadable
	...
	if err := repository.Save(
		...
		conflictingBinding,
		2,
	); !errors.Is(err, practicevoice.ErrAudioAssetAlreadyBound) {
	...
	bob.Status = practicevoice.AudioAssetMetadataCommitted
	bob.ETag = "etag-bob"
	...
	bob.Status = practicevoice.AudioAssetReadable
...
func TestAudioAssetRepositoryConcurrentCreateIsExactlyOnce(t *testing.T) {
	...
	for err := range errs {
		...
		case err == nil:
			created++
		case errors.Is(err, practicevoice.ErrAudioAssetConcurrentUpdate):
			conflicted++
		default:
...
func TestAudioAssetUploadClaimFencesWritesAndBlocksCleanup(t *testing.T) {
	...
	if first.Asset.ID != asset.ID ||
		first.Asset.Status != practicevoice.AudioAssetStaged ||
	...
	if _, err := repository.ClaimUpload(
		...
		asset.UploadRequestID,
		time.Minute,
	); !errors.Is(err, practicevoice.ErrAudioAssetConcurrentUpdate) {
	...
	ordinaryCommit.Status = practicevoice.AudioAssetMetadataCommitted
	...
	staleAfterRelease.Status = practicevoice.AudioAssetMetadataCommitted
	...
	secondCommit.Status = practicevoice.AudioAssetMetadataCommitted
	...
	completed.Status = practicevoice.AudioAssetMetadataCommitted
	...
	if err != nil ||
		persisted.Status != practicevoice.AudioAssetMetadataCommitted ||
...
func TestExpiredAmbiguousUploadCanBeClaimedForCleanup(t *testing.T) {
	...
	if len(cleanupClaims) != 1 ||
		...
		cleanupClaims[0].Asset.Status != practicevoice.AudioAssetDeleting ||
	...
	requestDeleting.Status = practicevoice.AudioAssetDeleting
...
func TestAudioAssetCleanupClaimsUseSkipLockedDatabaseClockAndFencing(
	...
	for _, asset := range []practicevoice.AudioAsset{locked, available, future, bob} {
		if err := repository.Create(ctx, asset); err != nil {
	...
	if availableClaim.Asset.Status != practicevoice.AudioAssetDeleting ||
	...
	expiredCompletion.Status = practicevoice.AudioAssetDeleted
	...
	wrongFenceCompletion.Status = practicevoice.AudioAssetDeleted
	...
	completed.Status = practicevoice.AudioAssetDeleted
	...
	if err != nil ||
		persisted.Status != practicevoice.AudioAssetDeleted ||
		persisted.Version != completed.Version {
	...
	ownerClaimsByID := make(map[string]practicevoice.AudioAssetCleanupClaim)
	...
	if err != nil || bobPersisted.Status != practicevoice.AudioAssetStaged {
...
func TestReleaseCleanupClaimMovesFailureBehindNextAsset(t *testing.T) {
	...
	for index, asset := range []*practicevoice.AudioAsset{&first, &second} {
		if err := repository.Create(ctx, *asset); err != nil {
		...
		*asset = persisted
		asset.Status = practicevoice.AudioAssetDeleting
	...
	staleAfterRelease.Status = practicevoice.AudioAssetDeleted
...
func TestRequestDeleteTakesOverExpiredCleanupLease(t *testing.T) {
	...
	requestDeleted.Status = practicevoice.AudioAssetDeleted
...
func TestAudioAssetUploadAndCleanupTolerateApplicationClockAheadOfDatabase(
	...
	uploaded.Status = practicevoice.AudioAssetMetadataCommitted
	...
	uploaded.Status = practicevoice.AudioAssetDeleting
	uploaded.Version++
	if err := repository.Save(
		ctx,
		uploaded,
...
func TestPurgeOwnerDeletedAssetsUnblocksIdentityDeletion(t *testing.T) {
	...
	ctx := context.Background()
	databaseNow := audioAssetDatabaseNow(t, pool)
	aliceAssets := []practicevoice.AudioAsset{
		newTestAudioAsset(
			"asset-alice-purge-a",
			audioTestOwnerA,
...
func TestAudioAssetMigrationEnforcesConstraintsIndexesAndDown(t *testing.T) {
	...
	rows, err := pool.Query(
		...
		`SELECT indexname
		 ...
		 WHERE schemaname = current_schema()
		   AND tablename = 'practice_audio_assets'`,
	)
	if err != nil {
	...
	for _, name := range []string{
		...
		"practice_audio_assets_owner_candidate_key",
		"practice_audio_assets_owner_turn_key",
		"practice_audio_assets_expired_cleanup_idx",
		"practice_audio_assets_upload_recovery_idx",
		"practice_audio_assets_deleting_cleanup_idx",
		"practice_audio_assets_owner_cleanup_idx",
		"practice_audio_assets_owner_deleted_purge_idx",
	} {
		if !indexes[name] {
	...
	if _, err := pool.Exec(
		...
		t.Fatalf("restore historical audio asset table name: %v", err)
	}
	...
	if err := pool.QueryRow(
		ctx,
		"SELECT to_regclass('practice_audio_assets')",
	).Scan(&relation); err != nil {
		t.Fatalf("inspect table after down migration: %v", err)
	}
	if relation != nil {
		t.Fatalf("practice_audio_assets remains after down migration as %q", *relation)
	}
}
...
func newAudioAssetIntegrationRepository(
	...
	if _, err := pool.Exec(
		...
		t.Fatalf("apply Practice audio asset authority fixture: %v", err)
	}
...
const practiceAudioAssetAuthorityFixtureSQL = `
...
func newTestAudioAsset(
	...
	return practicevoice.AudioAsset{
		...
		Status:          practicevoice.AudioAssetStaged,
