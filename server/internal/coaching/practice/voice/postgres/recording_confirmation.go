...
var _ practicevoice.RecordingConfirmationStore = (*Repository)(nil)
...
func (r *Repository) ConfirmTurnWithRecording(
...
) (practicevoice.RecordingConfirmation, error) {
	...
	if !validConfirmation(actor, command) ||
		...
		return practicevoice.RecordingConfirmation{},
			practicevoice.ErrPersistenceInvalid
	}

	tx, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return practicevoice.RecordingConfirmation{}, safeDatabaseError(err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	if _, err := ensureActorWritable(ctx, tx, actor.UserID); err != nil {
		return practicevoice.RecordingConfirmation{}, err
	}
	r.reachedWriteFence()
...
func bindRecordingInTransaction(
	...
	case practicevoice.AudioAssetMetadataCommitted:
		...
		if tag.RowsAffected() != 1 {
			return "", false, practicevoice.ErrAudioAssetConcurrentUpdate
		}
	...
	case practicevoice.AudioAssetReadable:
		...
		return "", false, practicevoice.ErrAudioAssetAlreadyBound
	case practicevoice.AudioAssetDeleting,
		...
		return "", false, practicevoice.ErrAudioAssetInvalidTransition
	default:
		return "", false, practicevoice.ErrAudioAssetInvalidTransition
	}
}
