...
import (
	...
	voicehttp "github.com/1024XEngineer/XE3-ESL/server/internal/coaching/practice/voice/http"
...
type audioAssetHTTPServiceStub struct {
	...
	playbackErr    error
	deleteResult   practicevoice.AudioAsset
	deleteErr      error
...
type audioAssetHTTPCall struct {
	actor   practicevoice.AudioAssetActor
	assetID string
}
...
func TestAudioAssetRoutesAcceptExact128ByteIdentifier(t *testing.T) {
	...
	service := &audioAssetHTTPServiceStub{
		deleteResult: practicevoice.AudioAsset{
			ID:     assetID,
			Status: practicevoice.AudioAssetDeleted,
		},
	}
	router := newAudioAssetHTTPRouter(t, service, trustedAlice)
...
func TestAudioAssetRoutesRejectNonCanonicalTrustedActor(t *testing.T) {
	...
	router := newAudioAssetHTTPRouter(t, service, func(*http.Request) (
		...
		return practicevoice.AudioAssetActor{UserID: " user-alice "}, true
	})
...
func TestAudioAssetPlaybackHidesAllUnavailableStatesAsNotFound(t *testing.T) {
	for name, serviceErr := range map[string]error{
		"missing":      practicevoice.ErrAudioAssetNotFound,
		"cross user":   practicevoice.ErrAudioAssetForbidden,
		"not readable": practicevoice.ErrAudioAssetInvalidTransition,
		"deleted":      practicevoice.ErrAudioAssetInvalidTransition,
		"malformed id": practicevoice.ErrAudioAssetInvalid,
	} {
		t.Run(name, func(t *testing.T) {
...
func TestAudioAssetDeleteReturns204OnlyAfterDeletionCompletes(t *testing.T) {
	...
	t.Run("still deleting", func(t *testing.T) {
		service := &audioAssetHTTPServiceStub{
			deleteResult: practicevoice.AudioAsset{
				ID:     "asset-1",
				Status: practicevoice.AudioAssetDeleting,
			},
		}
		router := newAudioAssetHTTPRouter(t, service, trustedAlice)
...
func TestAudioAssetDeleteHidesLookupAndProviderErrors(t *testing.T) {
	for name, test := range map[string]struct {
		...
		"missing": {
			serviceErr: practicevoice.ErrAudioAssetNotFound,
			status:     http.StatusNotFound,
			code:       "resource_not_found",
			message:    "Resource was not found.",
		},
		"cross user": {
			serviceErr: practicevoice.ErrAudioAssetForbidden,
			status:     http.StatusNotFound,
			code:       "resource_not_found",
			message:    "Resource was not found.",
		},
		...
		"concurrent lifecycle transition": {
			serviceErr: practicevoice.ErrAudioAssetConcurrentUpdate,
...
func newAudioAssetHTTPRouter(
	t *testing.T,
	service voicehttp.AudioAssetHTTPService,
	resolve voicehttp.AudioAssetActorResolverFunc,
) *gin.Engine {
	t.Helper()
	gin.SetMode(gin.TestMode)
	router := gin.New()
	if err := voicehttp.RegisterAudioAssetRoutes(router, service, resolve); err != nil {
		t.Fatalf("RegisterAudioAssetRoutes() error = %v", err)
	}
	return router
}

func trustedAlice(*http.Request) (practicevoice.AudioAssetActor, bool) {
	return practicevoice.AudioAssetActor{UserID: "user-alice"}, true
}
