...
type AudioAssetHTTPService interface {
	Playback(context.Context, practicevoice.AudioAssetActor, string) (objectstore.SignedGetResult, error)
	Delete(context.Context, practicevoice.AudioAssetActor, string) (practicevoice.AudioAsset, error)
}
...
type AudioAssetActorResolver interface {
	ResolveAudioAssetActor(*http.Request) (practicevoice.AudioAssetActor, bool)
}

// AudioAssetActorResolverFunc adapts an authentication function to
// AudioAssetActorResolver.
type AudioAssetActorResolverFunc func(*http.Request) (practicevoice.AudioAssetActor, bool)

func (resolve AudioAssetActorResolverFunc) ResolveAudioAssetActor(
	request *http.Request,
) (practicevoice.AudioAssetActor, bool) {
	return resolve(request)
}

// RegisterAudioAssetRoutes installs only the two protected routes frozen in
// the Practice Voice API. The composition root remains responsible for supplying
...
func RegisterAudioAssetRoutes(
	...
	if nilHTTPDependency(routes) || nilHTTPDependency(service) ||
		nilHTTPDependency(actors) {
		return practicevoice.ErrAudioAssetInvalidDependency
	}
...
func (handler audioAssetHTTPHandler) delete(c *gin.Context) {
	...
	if asset.Status != practicevoice.AudioAssetDeleted {
...
func (handler audioAssetHTTPHandler) resolveActor(
	...
	if !ok ||
		...
		!practicevoice.ValidAudioAssetIdentifier(actor.UserID) {
		...
		return practicevoice.AudioAssetActor{}, false
	}
	return actor, true
}
...
func (audioAssetHTTPHandler) resolveAssetID(c *gin.Context) (string, bool) {
	...
	if assetID != strings.TrimSpace(assetID) ||
		!practicevoice.ValidAudioAssetIdentifier(assetID) {
		writeAudioAssetHTTPError(
			c,
...
func (audioAssetHTTPHandler) writeServiceError(
	...
	if errors.Is(err, practicevoice.ErrAudioAssetConcurrentUpdate) ||
		errors.Is(err, practicevoice.ErrAudioAssetCleanupPending) {
		writeAudioAssetHTTPError(
			c,
			http.StatusConflict,
			"resource_conflict",
	...
	notFound := errors.Is(err, practicevoice.ErrAudioAssetNotFound) ||
		errors.Is(err, practicevoice.ErrAudioAssetForbidden) ||
		errors.Is(err, practicevoice.ErrAudioAssetInvalid)
	if hideInvalidState {
		notFound = notFound ||
			errors.Is(err, practicevoice.ErrAudioAssetInvalidTransition)
	}
	if notFound {
		writeAudioAssetHTTPError(
			c,
