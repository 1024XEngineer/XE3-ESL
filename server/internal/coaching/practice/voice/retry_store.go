...
var (
	ErrRetryTurnInvalid  = errors.New("practice voice: invalid retry Turn")
	ErrRetryTurnNotFound = errors.New("practice voice: retry Turn not found")
	ErrRetryTurnConflict = errors.New("practice voice: retry Turn conflict")
	ErrRetryTurnNotReady = errors.New("practice voice: retry Turn is not answering")
)
...
// RetryTurnDraft is a real Practice Voice Turn identity. The draft remains
...
func NewRetryTurnService(store RetryTurnStore) (*RetryTurnService, error) {
	if store == nil {
		return nil, errors.New(
			"practice voice: retry Turn store is required",
		)
	}
