...
var (
	_ practiceinput.AudioAssetLifecycleRepository = (*AudioAssetRepository)(nil)
	_ practiceinput.AudioAssetTurnVerifier        = (*AudioAssetRepository)(nil)
)
...
func NewAudioAssetRepository(
	...
	if pool == nil {
		return nil, practiceinput.ErrAudioAssetInvalidDependency
	}
...
func (r *AudioAssetRepository) VerifyOwnedTurn(
	...
	if !validOwnerID(ownerID) ||
		...
		return practiceinput.ErrAudioAssetInvalid
	}
	var exists bool
	err := r.pool.QueryRow(
		ctx,
	...
	if !exists {
		return practiceinput.ErrAudioAssetTurnNotFound
	}
	return nil
}
...
func (r *AudioAssetRepository) GetOwned(
	...
	if !validOwnerID(ownerID) ||
		...
		return practiceinput.AudioAsset{}, practiceinput.ErrAudioAssetInvalid
	}
	...
	if err != nil {
		return practiceinput.AudioAsset{}, mapAudioAssetReadError(err)
	}
...
func (r *AudioAssetRepository) ClaimUpload(
	...
	if !record.uploadLease.Valid || record.uploadFence <= 0 {
		return practiceinput.AudioAssetUploadClaim{}, ErrAudioAssetDatabase
	}
	return practiceinput.AudioAssetUploadClaim{
		Asset:          record.asset,
...
func (r *AudioAssetRepository) ClaimExpiredUnconfirmed(
	...
	leaseDuration time.Duration,
	limit int,
) ([]practiceinput.AudioAssetCleanupClaim, error) {
	leaseMicroseconds, valid := validLease(leaseDuration, limit)
	if !valid {
		return nil, practiceinput.ErrAudioAssetInvalid
	}
...
func (r *AudioAssetRepository) HasOwnerAssetsForAccountCleanup(
	...
	if !validOwnerID(ownerID) {
		return false, practiceinput.ErrAudioAssetInvalid
	}
...
func (r *AudioAssetRepository) PurgeOwnerDeletedAssets(
	...
	if !validOwnerID(ownerID) || !validLimit(limit) {
		return 0, practiceinput.ErrAudioAssetInvalid
	}
...
func (r *AudioAssetRepository) claimAudioAssets(
	...
	claims := make([]practiceinput.AudioAssetCleanupClaim, 0)
	for rows.Next() {
		...
		claims = append(claims, practiceinput.AudioAssetCleanupClaim{
			Asset:          record.asset,
...
func (r *AudioAssetRepository) classifyWriteMiss(
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return practiceinput.ErrAudioAssetNotFound
	}
	if err != nil {
		return safeAudioAssetDatabaseError(err)
	}
	return practiceinput.ErrAudioAssetConcurrentUpdate
}
...
func (r *AudioAssetRepository) reconcileUploadCommit(
	ctx context.Context,
	expected practiceinput.AudioAsset,
	fencingToken uint64,
) error {
	record, err := scanAudioAsset(r.pool.QueryRow(
		ctx,
	...
	if record.uploadFence == int64(fencingToken) &&
		...
		actual.Status == practiceinput.AudioAssetMetadataCommitted &&
...
func (r *AudioAssetRepository) reconcileUploadRelease(
	...
	if record.asset.Status == practiceinput.AudioAssetStaged &&
...
func (r *AudioAssetRepository) reconcileCleanupRelease(
	...
	if record.asset.Status == practiceinput.AudioAssetDeleting &&
...
func audioAssetsMatchPersisted(
	actual practiceinput.AudioAsset,
	expected practiceinput.AudioAsset,
) bool {
...
func validNewAudioAsset(asset practiceinput.AudioAsset) bool {
	return validPersistedAudioAsset(asset) &&
		asset.Status == practiceinput.AudioAssetStaged &&
		asset.CandidateID == "" &&
		asset.TurnID == "" &&
		asset.ETag == "" &&
...
func validPersistedAudioAsset(asset practiceinput.AudioAsset) bool {
	...
	switch asset.Status {
	case practiceinput.AudioAssetStaged:
	...
	case practiceinput.AudioAssetDeleted:
		return !asset.DeletedAt.IsZero() &&
...
func mapAudioAssetWriteError(err error) error {
	...
	if errors.As(err, &postgresError) {
		...
		case "practice_audio_assets_owner_candidate_key",
			...
			return practiceinput.ErrAudioAssetAlreadyBound
