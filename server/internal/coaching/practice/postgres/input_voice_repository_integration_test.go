...
func TestRepositoryRecoversQuestionCandidateAttemptAndTurn(t *testing.T) {
	...
	if err != nil ||
		completedReplay.Status != practiceinput.StoredTranscriptionCompleted ||
...
func TestReservationLeaseTakeoverRejectsOldWorkerCompleteAndFail(t *testing.T) {
	...
	err = repository.FailTranscription(
		context.Background(),
		oldJob,
		practiceinput.ProcessingFailure{Code: "provider_timeout", Retryable: true},
	)
	if !errors.Is(err, practiceinput.ErrPersistenceConflict) {
...
func TestCompletedCandidateRequiresStableProviderRequestID(t *testing.T) {
	...
	for _, providerRequestID := range []string{"", " \t"} {
		...
		if _, err := repository.CompleteTranscription(
			...
			job,
			command,
		); !errors.Is(err, practiceinput.ErrPersistenceInvalid) {
			t.Fatalf(
...
func TestConcurrentCandidatesForOneQuestionCreateOneFormalTurn(t *testing.T) {
	...
	candidates := make([]practiceinput.StoredTranscriptCandidate, 0, candidateCount)
	...
	for index, candidate := range candidates {
		commands = append(commands, practiceinput.ConfirmTurnCommand{
			CandidateID:     candidate.ID,
			EvidenceVersion: candidate.EvidenceVersion,
	...
	for range commands {
		...
		case outcome.err == nil:
			successes++
			winner = outcome
		case errors.Is(outcome.err, practiceinput.ErrPersistenceConflict):
			conflicts++
		default:
...
func TestInvalidOwnerAndDuplicateAddresseesAreRejected(t *testing.T) {
	repository, pool := newIntegrationRepository(t)
	invalidActor := practiceinput.Actor{
		UserID:    "not-a-uuid",
	...
	if _, err := repository.ReserveTranscription(
		...
		actor,
		command,
	); !errors.Is(err, practiceinput.ErrPersistenceNotFound) {
...
func TestFailureCodeBoundaryRejectsUnnormalizedValues(t *testing.T) {
	...
	for _, code := range []string{
		...
		if err := repository.FailTranscription(
			context.Background(),
			job,
			practiceinput.ProcessingFailure{Code: code, Retryable: true},
		); !errors.Is(err, practiceinput.ErrPersistenceInvalid) {
...
func TestActorIsolationDeletionAndGenerationFence(t *testing.T) {
	...
	for index, staleReadErr := range staleReadErrors {
		if !errors.Is(staleReadErr, practiceinput.ErrActorDeleted) {
			t.Fatalf(
...
func TestAccountDeletionSerializesWithConversationWrites(t *testing.T) {
	...
	t.Run("deletion commits before old worker", func(t *testing.T) {
		...
		if err := <-workerResult; !errors.Is(err, practiceinput.ErrPersistenceConflict) {
...
func testActor(userID string) practiceinput.Actor {
	return practiceinput.Actor{UserID: userID, SessionID: "trusted-session"}
}
...
func reserveCommand(
	...
	return practiceinput.StoreReserveTranscriptionCommand{
...
func reserveTestTranscription(
	...
	question practice.Question,
	key string,
) practiceinput.StoredTranscriptionReservation {
	t.Helper()
...
func completeCommand(transcriptID string) practiceinput.StoreCompleteTranscriptionCommand {
	return practiceinput.StoreCompleteTranscriptionCommand{
