...
func TestNewAudioAssetRepositoryRejectsNilPool(t *testing.T) {
	...
	if _, err := NewAudioAssetRepository(nil); !errors.Is(
		err,
		practiceinput.ErrAudioAssetInvalidDependency,
	) {
...
func TestAudioAssetRepositoryRejectsPaddedOpaqueIdentifiers(t *testing.T) {
	...
	for name, invoke := range tests {
		t.Run(name, func(t *testing.T) {
			if err := invoke(); !errors.Is(
				err,
				practiceinput.ErrAudioAssetInvalid,
			) {
	...
	if persisted.Status != practiceinput.AudioAssetStaged ||
		persisted.Version != 1 {
...
func TestAudioAssetRepositoryPersistsLifecycleAndOwnerScope(
	...
	if recovered.ID != alice.ID ||
		...
		recovered.Status != practiceinput.AudioAssetStaged ||
	...
	if _, err := restarted.GetOwned(
		...
		audioTestOwnerB,
		alice.ID,
	); !errors.Is(err, practiceinput.ErrAudioAssetNotFound) {
	...
	if err := repository.Create(ctx, duplicateRequest); !errors.Is(
		err,
		practiceinput.ErrAudioAssetConcurrentUpdate,
	) {
	...
	alice.Status = practiceinput.AudioAssetMetadataCommitted
	...
	alice.Status = practiceinput.AudioAssetReadable
	...
	conflictingBinding.Status = practiceinput.AudioAssetMetadataCommitted
	...
	conflictingBinding.Status = practiceinput.AudioAssetReadable
	...
	bob.Status = practiceinput.AudioAssetMetadataCommitted
	bob.ETag = "etag-bob"
	...
	bob.Status = practiceinput.AudioAssetReadable
...
func TestAudioAssetUploadClaimFencesWritesAndBlocksCleanup(t *testing.T) {
	...
	if first.Asset.ID != asset.ID ||
		first.Asset.Status != practiceinput.AudioAssetStaged ||
	...
	if _, err := repository.ClaimUpload(
		...
		asset.UploadRequestID,
		time.Minute,
	); !errors.Is(err, practiceinput.ErrAudioAssetConcurrentUpdate) {
	...
	ordinaryCommit.Status = practiceinput.AudioAssetMetadataCommitted
	...
	staleAfterRelease.Status = practiceinput.AudioAssetMetadataCommitted
	...
	secondCommit.Status = practiceinput.AudioAssetMetadataCommitted
	...
	completed.Status = practiceinput.AudioAssetMetadataCommitted
	...
	if err != nil ||
		persisted.Status != practiceinput.AudioAssetMetadataCommitted ||
...
func TestExpiredAmbiguousUploadCanBeClaimedForCleanup(t *testing.T) {
	...
	if len(cleanupClaims) != 1 ||
		...
		cleanupClaims[0].Asset.Status != practiceinput.AudioAssetDeleting ||
	...
	requestDeleting.Status = practiceinput.AudioAssetDeleting
...
func TestAudioAssetCleanupClaimsUseSkipLockedDatabaseClockAndFencing(
	...
	for _, asset := range []practiceinput.AudioAsset{locked, available, future, bob} {
		if err := repository.Create(ctx, asset); err != nil {
	...
	if availableClaim.Asset.Status != practiceinput.AudioAssetDeleting ||
	...
	expiredCompletion.Status = practiceinput.AudioAssetDeleted
	...
	wrongFenceCompletion.Status = practiceinput.AudioAssetDeleted
	...
	completed.Status = practiceinput.AudioAssetDeleted
	...
	if err != nil ||
		persisted.Status != practiceinput.AudioAssetDeleted ||
		persisted.Version != completed.Version {
	...
	ownerClaimsByID := make(map[string]practiceinput.AudioAssetCleanupClaim)
	...
	if err != nil || bobPersisted.Status != practiceinput.AudioAssetStaged {
...
func TestReleaseCleanupClaimMovesFailureBehindNextAsset(t *testing.T) {
	...
	for index, asset := range []*practiceinput.AudioAsset{&first, &second} {
		if err := repository.Create(ctx, *asset); err != nil {
		...
		*asset = persisted
		asset.Status = practiceinput.AudioAssetDeleting
	...
	staleAfterRelease.Status = practiceinput.AudioAssetDeleted
...
func TestRequestDeleteTakesOverExpiredCleanupLease(t *testing.T) {
	...
	requestDeleted.Status = practiceinput.AudioAssetDeleted
...
func TestAudioAssetUploadAndCleanupTolerateApplicationClockAheadOfDatabase(
	...
	uploaded.Status = practiceinput.AudioAssetMetadataCommitted
	...
	uploaded.Status = practiceinput.AudioAssetDeleting
	uploaded.Version++
	if err := repository.Save(
		ctx,
		uploaded,
...
func TestPurgeOwnerDeletedAssetsUnblocksIdentityDeletion(t *testing.T) {
	...
	ctx := context.Background()
	databaseNow := audioAssetDatabaseNow(t, pool)
	aliceAssets := []practiceinput.AudioAsset{
		newTestAudioAsset(
			"asset-alice-purge-a",
			audioTestOwnerA,
...
func newTestAudioAsset(
	...
	return practiceinput.AudioAsset{
		...
		Status:          practiceinput.AudioAssetStaged,
