...
var _ practiceinput.RetryTurnStore = (*Repository)(nil)
...
func (r *Repository) CreateRetryTurn(
	...
	if r == nil || r.pool == nil || ctx == nil || !actor.Valid() ||
		...
		return practiceinput.RetryTurnDraft{},
			practiceinput.ErrRetryTurnInvalid
	}
	tx, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return practiceinput.RetryTurnDraft{}, safeDatabaseError(err)
	}
	...
	if err := lockKey(
		...
		return practiceinput.RetryTurnDraft{}, err
	}
	...
	if err == nil {
		if existing.PracticeSessionID != command.PracticeSessionID ||
			...
			return practiceinput.RetryTurnDraft{},
				practiceinput.ErrRetryTurnConflict
		}
	...
	if !errors.Is(err, practiceinput.ErrRetryTurnNotFound) {
		return practiceinput.RetryTurnDraft{}, err
	}
...
func scanRetryTurn(
	...
	var draft practiceinput.RetryTurnDraft
	var status string
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return practiceinput.RetryTurnDraft{},
			practiceinput.ErrRetryTurnNotFound
	}
	if err != nil {
		return practiceinput.RetryTurnDraft{},
			safeDatabaseError(err)
	}
	draft.Status = practiceinput.RetryTurnStatus(status)
...
func mapRetryTurnError(err error) error {
	...
	case errors.Is(err, practiceinput.ErrPersistenceNotFound),
		...
		return practiceinput.ErrRetryTurnNotFound
	case errors.Is(err, practiceinput.ErrPersistenceConflict):
		return practiceinput.ErrRetryTurnConflict
	default:
		return err
	}
}
