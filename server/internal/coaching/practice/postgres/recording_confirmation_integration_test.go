...
func TestConfirmTurnWithRecordingExcludesCleanupAndReplaysAfterRestart(
	...
	type confirmationOutcome struct {
		result practiceinput.RecordingConfirmation
		err    error
	}
...
func TestConfirmTurnWithRecordingRollsBackWhenCleanupWins(
	...
	for attempt := range 2 {
		...
		if !errors.Is(
			confirmErr,
			practiceinput.ErrAudioAssetInvalidTransition,
		) {
			t.Fatalf(
...
func TestConfirmTurnWithRecordingReplaysTurnWithoutDeletedRecording(
	...
	deleting := readable
	deleting.Status = practiceinput.AudioAssetDeleting
...
func createRecordingConfirmationFixture(
	...
	committed.Status = practiceinput.AudioAssetMetadataCommitted
	...
	return actor, candidate, practiceinput.ConfirmTurnCommand{
