...
func mapPracticeSession(
	...
	if session.ID == "" ||
		...
		snapshot.PlanVersion != session.PlanVersion ||
	...
	result := Session{
		...
		Background:                 snapshot.Preparation.BackgroundSnapshot,
...
func cloneInterviewQuestionContext(
	...
	if preparation.JobTargetInputSnapshot == nil &&
		...
		preparation.ResumeMaterial == nil {
		return nil
	}
	...
	if resume := preparation.ResumeMaterial; resume != nil {
		material := *resume
		material.WorkExperiences = make(
			[]practice.ResumeWorkExperience,
			len(resume.WorkExperiences),
		)
		for index, work := range resume.WorkExperiences {
		...
		material.ProjectExperiences = make(
			...
			len(resume.ProjectExperiences),
		)
		for index, project := range resume.ProjectExperiences {
		...
		material.Skills = cloneVoiceStrings(resume.Skills)
		material.Awards = cloneVoiceStrings(resume.Awards)
...
func cloneIELTSAssignment(
	...
	for index, part := range source.Parts {
		...
		result.Parts[index].PreparedAnswers = append(
			[]practice.IELTSPreparedAnswer(nil),
