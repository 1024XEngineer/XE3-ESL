...
func (service *RoundService) TranscribeStream(
	ctx context.Context,
	...
	if !ok {
		err = NewProviderError(
			...
			"",
			errors.New("practice interaction: streaming recognizer is required"),
		)
...
func (service *RoundService) captureRealtimePCM(
	ctx context.Context,
	actor requestcontext.Actor,
	pcm []byte,
	sampleRate int,
