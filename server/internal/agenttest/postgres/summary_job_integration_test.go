...
func TestCompletedRunQueuesAndProcessesThreadSummaryJob(t *testing.T) {
	...
	checkpoint, err := repository.summary.FindLatestCheckpoint(
		context.Background(),
		agentTestUserA,
		thread.ID,
		40,
...
func TestThreadSummaryJobSkipsBelowThresholdAndSerializesThreadClaims(
	...
	for range 2 {
		...
		go func() {
			defer wait.Done()
			<-start
			claim, acquired, claimErr := repository.summary.ClaimJob(
				context.Background(),
				configuration,
			)
			results <- claimResult{
	...
	if _, err := repository.summary.FinishJob(
		context.Background(),
		firstClaim,
...
func TestThreadSummaryJobRecoversExpiredLeaseAndExhaustsRetries(
	...
	configuration := summaryWorkerConfiguration()
	firstClaim, acquired, err := repository.summary.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
...
WHERE source_run_id = $1`,
	...
	recovered, acquired, err := repository.summary.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
	...
	retried, err := repository.summary.FailJob(
		context.Background(),
		recovered,
		0,
		"dependency",
...
WHERE source_run_id = $1`,
	...
	finalClaim, acquired, err := repository.summary.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
	...
	failed, err := repository.summary.FailJob(
		context.Background(),
		finalClaim,
		0,
		"dependency",
