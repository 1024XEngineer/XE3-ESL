...
func TestPostgresSummaryCheckpointChainVisibilityAndOwnership(t *testing.T) {
	...
	first, err := repository.summary.CreateCheckpoint(
		ctx,
	...
	if _, err := repository.summary.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		1,
	...
	second, err := repository.summary.CreateCheckpoint(
		ctx,
	...
	visibleAtThree, err := repository.summary.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		3,
	...
	visibleAtFour, err := repository.summary.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		4,
...
func TestPostgresSummaryCheckpointRejectsInvalidChainAndFutureCoverage(
	...
	if _, err := repository.summary.CreateCheckpoint(
		ctx,
		future,
	); !errors.Is(err, conversation.ErrInvalidRequest) {
...
func TestPostgresSummaryCheckpointConcurrentCreateDoesNotFork(t *testing.T) {
	...
	for range 2 {
		...
		go func() {
			defer wait.Done()
			<-start
			_, createErr := repository.summary.CreateCheckpoint(ctx, command)
			results <- createErr
		}()
	}
	close(start)
	...
	for createErr := range results {
		...
		case createErr == nil:
			successes++
		case errors.Is(createErr, conversation.ErrConflict):
			conflicts++
		default:
...
func TestPostgresSummarySourceAndGenerationService(t *testing.T) {
	...
	messages, err := repository.summary.ListMessagesForSummary(
		ctx,
		agentTestUserA,
		thread.ID,
		1,
	...
	if len(messages) != 2 ||
		...
		messages[1].Role != conversation.MessageRoleUser {
		t.Fatalf("unexpected summary source: %#v", messages)
	}
	if _, err := repository.summary.ListMessagesForSummary(
		ctx,
		agentTestUserB,
		thread.ID,
		1,
	...
	first, err := service.GenerateCheckpoint(
		ctx,
		agentsummary.GenerateCheckpointCommand{
...
func TestPostgresSummarySourceRejectsCharacterOverflow(t *testing.T) {
	...
	summaryRepository, err := summarypostgres.New(
		database.pool,
...
INSERT INTO agent_messages (
	...
	if _, err := summaryRepository.ListMessagesForSummary(
		ctx,
		agentTestUserA,
		thread.ID,
		1,
...
func summaryCommand(
	...
	source string,
) agentsummary.CreateCheckpointCommand {
	return agentsummary.CreateCheckpointCommand{
		OwnerID:                agentTestUserA,
		ThreadID:               threadID,
