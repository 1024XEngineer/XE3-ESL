...
func (s EvidenceSnapshot) Valid() bool {
	canonical, err := canonicalEvidencePayload(s.Payload)
	sourceManifestHash, sourceErr := evidenceSourceManifestHash(canonical)
...
func normalizeEvidenceSnapshotCommand(
	...
	if command.SnapshotID != deriveEvidenceSnapshotID(
		command.OwnerUserID,
	...
	canonical, err := canonicalEvidencePayload(command.CanonicalPayload)
	if err != nil ||
		!evidencePayloadMatchesSnapshot(
			canonical,
			command.SnapshotID,
	...
	sourceManifestHash, err := evidenceSourceManifestHash(canonical)
...
func evidencePayloadMatchesSnapshot(
	...
	versionByTurn := make(
		map[string]evidenceTurnVersion,
		len(evidence.VersionManifest.TurnEvidence),
	)
...
func typedEvidencePayloadMatchesCanonical(
	...
	canonical, err := canonicalEvidencePayload(encoded)
...
func validEvidenceRefBinding(
	...
	expectedRefID := stableEvidenceRefID(
		snapshotID,
		turn.TurnID,
...
func opportunitiesBindToTurns(
	...
	turns map[string]evidenceConfirmedTurn,
...
func evidencePracticeContextMatchesScene(
	context evidencePracticeContext,
	sceneType SceneType,
) bool {
...
func validEvidenceUnavailableDeclarations(evidence evidencePayload) bool {
