// Package postgres implements Conversation's PostgreSQL persistence adapters.
package postgres
...
const (
	...
	// retention promise. The Conversation service currently uses 24 hours.
...
func (r *AudioAssetRepository) GetOwned(
	...
	ownerID string,
	audioAssetID string,
) (conversation.AudioAsset, error) {
	if !validOwnerID(ownerID) ||
		!validAudioAssetIdentifier(audioAssetID) {
		return conversation.AudioAsset{}, conversation.ErrAudioAssetInvalid
	}
	...
	if err != nil {
		return conversation.AudioAsset{}, mapAudioAssetReadError(err)
	}
...
func (r *AudioAssetRepository) ClaimUpload(
	...
	record, err := scanAudioAsset(r.pool.QueryRow(
		...
		UPDATE conversation_audio_assets AS asset
	...
	if !record.uploadLease.Valid || record.uploadFence <= 0 {
		return conversation.AudioAssetUploadClaim{}, ErrAudioAssetDatabase
	}
	return conversation.AudioAssetUploadClaim{
		Asset:          record.asset,
...
func (r *AudioAssetRepository) CommitUploadClaim(
	...
	if !validPersistedAudioAsset(asset) ||
		asset.Status != conversation.AudioAssetMetadataCommitted ||
		expectedVersion == 0 ||
		expectedVersion >= uint64(^uint64(0)>>1) ||
		asset.Version != expectedVersion+1 ||
		fencingToken == 0 ||
...
func (r *AudioAssetRepository) ClaimExpiredUnconfirmed(
	...
	leaseDuration time.Duration,
	limit int,
) ([]conversation.AudioAssetCleanupClaim, error) {
	leaseMicroseconds, valid := validLease(leaseDuration, limit)
	if !valid {
		return nil, conversation.ErrAudioAssetInvalid
	}
...
func (r *AudioAssetRepository) HasOwnerAssetsForAccountCleanup(
	...
	if !validOwnerID(ownerID) {
		return false, conversation.ErrAudioAssetInvalid
	}
...
func (r *AudioAssetRepository) PurgeOwnerDeletedAssets(
	...
	if !validOwnerID(ownerID) || !validLimit(limit) {
		return 0, conversation.ErrAudioAssetInvalid
	}
	...
	commandTag, err := r.pool.Exec(
		...
		DELETE FROM conversation_audio_assets AS asset
		USING selected
...
func (r *AudioAssetRepository) claimAudioAssets(
	...
	claims := make([]conversation.AudioAssetCleanupClaim, 0)
	for rows.Next() {
		...
		claims = append(claims, conversation.AudioAssetCleanupClaim{
			Asset:          record.asset,
...
func (r *AudioAssetRepository) classifyWriteMiss(
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return conversation.ErrAudioAssetNotFound
	}
	if err != nil {
		return safeAudioAssetDatabaseError(err)
	}
	return conversation.ErrAudioAssetConcurrentUpdate
}
...
func (r *AudioAssetRepository) reconcileUploadCommit(
	ctx context.Context,
	expected conversation.AudioAsset,
	fencingToken uint64,
) error {
	record, err := scanAudioAsset(r.pool.QueryRow(
		ctx,
	...
	if record.uploadFence == int64(fencingToken) &&
		...
		actual.Status == conversation.AudioAssetMetadataCommitted &&
...
func (r *AudioAssetRepository) reconcileUploadRelease(
	...
	if record.asset.Status == conversation.AudioAssetStaged &&
...
func (r *AudioAssetRepository) reconcileCleanupRelease(
	...
	if record.asset.Status == conversation.AudioAssetDeleting &&
...
func (r *AudioAssetRepository) getOwnedRecord(
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return audioAssetRecord{}, conversation.ErrAudioAssetNotFound
	}
	if err != nil {
...
func audioAssetsMatchPersisted(
	actual conversation.AudioAsset,
	expected conversation.AudioAsset,
) bool {
...
func validNewAudioAsset(asset conversation.AudioAsset) bool {
	return validPersistedAudioAsset(asset) &&
		asset.Status == conversation.AudioAssetStaged &&
		asset.CandidateID == "" &&
		asset.TurnID == "" &&
		asset.ETag == "" &&
...
func validPersistedAudioAsset(asset conversation.AudioAsset) bool {
	...
	switch asset.Status {
	case conversation.AudioAssetStaged:
		return asset.CandidateID == "" &&
			asset.TurnID == "" &&
			asset.DeletedAt.IsZero()
	case conversation.AudioAssetMetadataCommitted:
		return strings.TrimSpace(asset.ETag) != "" &&
			asset.CandidateID == "" &&
			asset.TurnID == "" &&
			asset.DeletedAt.IsZero()
	case conversation.AudioAssetReadable:
		return strings.TrimSpace(asset.ETag) != "" &&
			asset.CandidateID != "" &&
			asset.TurnID != "" &&
			asset.DeletedAt.IsZero()
	case conversation.AudioAssetDeleting:
		return asset.DeletedAt.IsZero()
	case conversation.AudioAssetDeleted:
		return !asset.DeletedAt.IsZero() &&
...
func mapAudioAssetWriteError(err error) error {
	...
	if errors.As(err, &postgresError) {
		...
		case "conversation_audio_assets_owner_candidate_key",
			"conversation_audio_assets_owner_turn_key":
			return conversation.ErrAudioAssetAlreadyBound
		case "conversation_audio_assets_pkey",
