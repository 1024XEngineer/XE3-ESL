...
type audioAssetHTTPServiceStub struct {
	...
	playbackErr    error
	deleteResult   conversation.AudioAsset
	deleteErr      error
...
type audioAssetHTTPCall struct {
	actor   conversation.AudioAssetActor
	assetID string
}
...
func TestAudioAssetRoutesRequireAuthenticatedActor(t *testing.T) {
	...
	router := newAudioAssetHTTPRouter(t, service, func(*http.Request) (
		...
		return conversation.AudioAssetActor{}, false
	})
...
func TestAudioAssetRoutesAcceptExact128ByteIdentifier(t *testing.T) {
	...
	service := &audioAssetHTTPServiceStub{
		deleteResult: conversation.AudioAsset{
			ID:     assetID,
			Status: conversation.AudioAssetDeleted,
		},
	}
	router := newAudioAssetHTTPRouter(t, service, trustedAlice)
...
func TestAudioAssetRoutesRejectNonCanonicalTrustedActor(t *testing.T) {
	...
	router := newAudioAssetHTTPRouter(t, service, func(*http.Request) (
		...
		return conversation.AudioAssetActor{UserID: " user-alice "}, true
	})
...
func TestAudioAssetPlaybackHidesAllUnavailableStatesAsNotFound(t *testing.T) {
	for name, serviceErr := range map[string]error{
		"missing":      conversation.ErrAudioAssetNotFound,
		"cross user":   conversation.ErrAudioAssetForbidden,
		"not readable": conversation.ErrAudioAssetInvalidTransition,
		"deleted":      conversation.ErrAudioAssetInvalidTransition,
		"malformed id": conversation.ErrAudioAssetInvalid,
	} {
		t.Run(name, func(t *testing.T) {
...
func TestAudioAssetDeleteReturns204OnlyAfterDeletionCompletes(t *testing.T) {
	...
	t.Run("still deleting", func(t *testing.T) {
		service := &audioAssetHTTPServiceStub{
			deleteResult: conversation.AudioAsset{
				ID:     "asset-1",
				Status: conversation.AudioAssetDeleting,
			},
		}
		router := newAudioAssetHTTPRouter(t, service, trustedAlice)
...
func TestAudioAssetDeleteHidesLookupAndProviderErrors(t *testing.T) {
	for name, test := range map[string]struct {
		...
		"missing": {
			serviceErr: conversation.ErrAudioAssetNotFound,
			status:     http.StatusNotFound,
			code:       "resource_not_found",
			message:    "Resource was not found.",
		},
		"cross user": {
			serviceErr: conversation.ErrAudioAssetForbidden,
			status:     http.StatusNotFound,
			code:       "resource_not_found",
			message:    "Resource was not found.",
		},
		...
		"concurrent lifecycle transition": {
			serviceErr: conversation.ErrAudioAssetConcurrentUpdate,
...
func newAudioAssetHTTPRouter(
	t *testing.T,
	service conversation.AudioAssetHTTPService,
	resolve conversation.AudioAssetActorResolverFunc,
) *gin.Engine {
	t.Helper()
	gin.SetMode(gin.TestMode)
	router := gin.New()
	if err := conversation.RegisterAudioAssetRoutes(router, service, resolve); err != nil {
		t.Fatalf("RegisterAudioAssetRoutes() error = %v", err)
	}
	return router
}

func trustedAlice(*http.Request) (conversation.AudioAssetActor, bool) {
	return conversation.AudioAssetActor{UserID: "user-alice"}, true
}
