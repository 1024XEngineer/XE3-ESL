...
func TestSpeechFeedbackAnchorUsesExactUTF8ByteOffsets(t *testing.T) {
	...
	if !anchor.validFor(source, "review_evidence_1", text) {
	...
	if insideRune.validFor(source, "review_evidence_1", text) {
		t.Fatal("anchor starting inside a UTF-8 rune was accepted")
	}

	wrongExcerpt := anchor
	wrongExcerpt.OriginalExcerpt = "Hello"
	if wrongExcerpt.validFor(source, "review_evidence_1", text) {
