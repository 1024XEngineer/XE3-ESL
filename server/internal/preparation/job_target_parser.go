...
func NewAIJobTargetParser(
	...
	if generator == nil || catalog == nil {
		return nil, errors.New(
			"preparation: job target parser dependency is required",
		)
	}
	manifest, err := jobTargetCatalogManifest(catalog)
	if err != nil {
		return nil, err
	}
...
type jobTargetCatalogManifestDocument struct {
	Scenarios []jobTargetCatalogScenario `json:"scenarios"`
}
...
func jobTargetCatalogManifest(catalog CatalogReader) (string, error) {
	definitions := catalog.ListActiveScenarios()
	...
	for _, definition := range definitions {
		...
		roles, err := catalog.ListRoles(definition.ID)
		if err != nil {
			return "", errors.New(
				"preparation: job target parser catalog is inconsistent",
			)
		}
		scenario := jobTargetCatalogScenario{
			...
			PracticeOptions: clonePracticeOptions(
				detail.PracticeOptions,
			),
		}
		for _, role := range roles {
			scenario.Roles = append(
				scenario.Roles,
				jobTargetCatalogRole{
