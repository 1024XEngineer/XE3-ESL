...
func TestLiveObjectLifecycle(t *testing.T) {
	if os.Getenv("OSS_LIVE_TEST") != "1" {
		t.Skip("set OSS_LIVE_TEST=1 for an explicit real OSS lifecycle test")
	}
	...
	key := fmt.Sprintf("%s/live-tests/%s.wav", storageConfig.AudioPrefix, randomID(t))
	...
	if readErr != nil || closeErr != nil {
		t.Fatalf("read live object: read=%v close=%v", readErr, closeErr)
	}
	if response.StatusCode != http.StatusOK || !bytes.Equal(downloaded, payload) {
		t.Fatalf("downloaded live object mismatch: status=%d size=%d", response.StatusCode, len(downloaded))
	}
	...
	if err != nil {
		t.Fatalf("verify deleted live object: %v", err)
	}
	_, _ = io.Copy(io.Discard, io.LimitReader(deletedResponse.Body, 4096))
