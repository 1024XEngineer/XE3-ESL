...
func TestPostgresContextAssemblerSelectsSummaryAndExcludesCoveredMessages(
	...
	checkpoint, err := repository.CreateSummaryCheckpoint(
		context.Background(),
	...
	if manifest.SummaryContextPolicyVersion != summaryContextPolicyV1 ||
		manifest.SummaryContextStatus != summaryContextSelected ||
...
func TestPostgresContextAssemblerAuditsSummaryOmittedByBudget(
	...
	if manifest.SummaryContextStatus != summaryContextOmittedBudget ||
		manifest.SelectedSummary != nil ||
...
func TestPostgresContextAssemblerCombinesSummaryAndBudgetTrim(
	...
	if manifest.SummaryContextStatus != summaryContextSelected ||
		manifest.SelectedSummary == nil ||
		manifest.SelectedSummary.CheckpointID != checkpoint.ID ||
