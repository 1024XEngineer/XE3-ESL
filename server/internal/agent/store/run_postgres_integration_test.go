...
WHERE table_schema = current_schema()
  ...
  AND column_name IN (
	...
	handler, err := agenttransport.NewHTTPHandlerWithRuns(
		dataService,
		runService,
		matterService,
...
func TestPostgresAgentRunPendingClaimHasOneWinner(t *testing.T) {
	...
	submission, err := repository.CreateInitial(
		context.Background(),
		actor.UserID,
		thread.ID,
		"claim-race-message",
	...
	for range claimers {
		...
		go func() {
			...
			<-start
			run, won, claimErr := repository.Claim(
				context.Background(),
				actor.UserID,
				submission.Run.ID,
			)
...
func TestPostgresAgentRunRollsBackUserMessageWhenPendingRunCannotCommit(
	...
	if _, err := repository.CreateInitial(
		context.Background(),
		actor.UserID,
		thread.ID,
...
func TestPostgresAgentRunPanicRollsBackAndReleasesConnection(t *testing.T) {
	...
	panicRepository, err := agentstore.NewPostgresStore(
		database.pool,
...
WHERE id = $1 AND owner_user_id = $2`,
	...
	claimed, acquired, err := repository.Claim(
		context.Background(),
		actor.UserID,
		submission.Run.ID,
	)
...
func TestPostgresAgentRunRecoversRunningAndResumesPendingAfterRestart(
	...
	running, err := repository.CreateInitial(
		context.Background(),
		actor.UserID,
		runningThread.ID,
	...
	pending, err := repository.CreateInitial(
		context.Background(),
		actor.UserID,
		pendingThread.ID,
...
WHERE id = $1 AND owner_user_id = $2`,
	...
	if _, err := recoveredRepository.Complete(
		context.Background(),
		actor.UserID,
		running.Run.ID,
		claimed.WorkerLeaseToken,
...
func TestPostgresContextAssemblerNormalizesOnlyMemorySearchQuery(
	...
	dataService, err := conversation.NewService(repository, matterService)
	if err != nil {
...
func TestPostgresAgentMemoryStoresIndexesRecallsAndInjects(t *testing.T) {
	...
	dataService, err := conversation.NewService(agentRepository, matterService)
	if err != nil {
		t.Fatalf("new Agent service: %v", err)
	}
	generator := &recordingTextGenerator{result: successfulTextResult()}
...
WHERE memory_id = $1`,
	...
	freshAgentRepository, err := agentstore.NewPostgresStore(database.pool, ids)
	if err != nil {
...
) (*matter.Service, *conversation.Service, *agentrun.Service, *agentstore.PostgresStore) {
	t.Helper()
	ids := identity.NewUUIDv4Generator(nil)
...
func newRunService(
	t *testing.T,
	repository *agentstore.PostgresStore,
...
func newRunCapableStore(
	...
	t.Helper()
	base, err := agentstore.NewPostgresStore(pool, ids)
	if err != nil {
