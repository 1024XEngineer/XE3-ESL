...
func TestCompletedRunQueuesAndProcessesThreadSummaryJob(t *testing.T) {
	...
	checkpoint, err := repository.FindLatestCheckpoint(
		context.Background(),
		agentTestUserA,
		thread.ID,
		40,
...
func TestThreadSummaryJobSkipsBelowThresholdAndSerializesThreadClaims(
	...
	for range 2 {
		...
		go func() {
			defer wait.Done()
			<-start
			claim, acquired, claimErr := repository.ClaimJob(
				context.Background(),
				configuration,
			)
			results <- claimResult{
...
func TestThreadSummaryJobRecoversExpiredLeaseAndExhaustsRetries(
	...
	configuration := summaryWorkerConfiguration()
	firstClaim, acquired, err := repository.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
...
WHERE source_run_id = $1`,
	...
	recovered, acquired, err := repository.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
	...
	retried, err := repository.FailJob(
		context.Background(),
		recovered,
		0,
		"dependency",
...
WHERE source_run_id = $1`,
	...
	finalClaim, acquired, err := repository.ClaimJob(
		context.Background(),
		configuration,
	)
	if err != nil || !acquired {
