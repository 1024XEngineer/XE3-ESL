...
func TestPostgresSummaryCheckpointChainVisibilityAndOwnership(t *testing.T) {
	...
	first, err := repository.CreateCheckpoint(
		ctx,
	...
	if _, err := repository.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		1,
	...
	second, err := repository.CreateCheckpoint(
		ctx,
	...
	visibleAtThree, err := repository.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		3,
	...
	visibleAtFour, err := repository.FindLatestCheckpoint(
		ctx,
		agentTestUserA,
		thread.ID,
		4,
...
func TestPostgresSummaryCheckpointRejectsInvalidChainAndFutureCoverage(
	...
	if _, err := repository.CreateCheckpoint(
		ctx,
		future,
...
func TestPostgresSummaryCheckpointConcurrentCreateDoesNotFork(t *testing.T) {
	...
	for range 2 {
		...
		go func() {
			defer wait.Done()
			<-start
			_, createErr := repository.CreateCheckpoint(ctx, command)
			results <- createErr
		}()
	}
	close(start)
...
func TestPostgresSummarySourceRejectsCharacterOverflow(t *testing.T) {
	...
	repository, err := agentstore.NewPostgresStore(
		database.pool,
