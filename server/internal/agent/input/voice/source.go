...
func NewSignedAudioLoader(
	store objectstore.Store,
	client *http.Client,
	scratchDirectory string,
	allowedHosts []string,
) (*SignedAudioLoader, error) {
	return newSignedAudioLoader(
		store,
		client,
		scratchDirectory,
		allowedHosts,
...
func newSignedAudioLoader(
	...
	if client.Timeout <= 0 || resolver == nil {
		return nil, errors.New("agent voice input: object client timeout is required")
	}
	safeClient := *client
	...
	for _, allowedHost := range allowedHosts {
		...
		if !ok {
			return nil, errors.New(
				"agent voice input: object allowed host is invalid",
			)
		}
	...
	if len(trustedHosts) == 0 {
		return nil, errors.New(
			"agent voice input: object allowed host is required",
		)
	}
...
func (loader *SignedAudioLoader) validateSignedTarget(
	ctx context.Context,
	target *url.URL,
) error {
...
var _ AudioSourceLoader = (*SignedAudioLoader)(nil)
