...
func (s *Service) Upload(
	...
	if ctx == nil || !actor.Valid() || !core.ValidUUID(request.ThreadID) ||
	...
	if len(payload) > core.MaxImageBytes {
	...
	now := s.clock()
	stage, err := s.repository.StageImageAsset(ctx, Asset{
		ID:              assetID,
		OwnerID:         actor.UserID,
		ThreadID:        request.ThreadID,
		UploadRequestID: request.IdempotencyKey,
		ObjectKey:       core.AgentImageObjectPrefix + assetID + normalized.extension,
		ContentType:     normalized.contentType,
		Size:            int64(len(normalized.payload)),
		Width:           normalized.width,
		Height:          normalized.height,
	...
	claim, acquired, err := s.repository.ClaimImageUpload(
		ctx,
		actor.UserID,
		asset.ID,
	...
	return s.repository.CommitImageUpload(
		ctx,
		actor.UserID,
		claim.Asset.ID,
		claim.FencingToken,
...
func (s *Service) Get(
	...
	if ctx == nil || !actor.Valid() || !core.ValidUUID(assetID) {
		return Asset{}, ErrNotFound
	}
	return s.repository.FindImageAsset(ctx, actor.UserID, assetID)
}
...
func (s *Service) MessageImages(
	...
	if ctx == nil || !actor.Valid() || !core.ValidUUID(threadID) ||
		!core.ValidUUID(messageID) {
		return nil, ErrNotFound
	}
	assets, err := s.repository.ListMessageImageAssets(
		ctx,
		actor.UserID,
		threadID,
		messageID,
...
func (s *Service) MessageAssets(
	...
	return s.repository.ListMessageImageAssets(
		ctx,
		actor.UserID,
		threadID,
		messageID,
...
func (s *Service) Attach(
	...
	return s.repository.AttachImageAssets(
		ctx,
		actor.UserID,
		threadID,
		messageID,
...
func (s *Service) Delete(
	...
	asset, err := s.repository.BeginImageAssetDeletion(
		ctx,
		actor.UserID,
		assetID,
	)
	...
	_, err = s.repository.FinishImageAssetDeletion(
		ctx,
		actor.UserID,
		asset.ID,
	)
...
func (s *Service) Reclaim(
	...
	claims, err := s.repository.ClaimImageCleanup(
		ctx,
		defaultCleanupLease,
		limit,
	)
	...
	for _, claim := range claims {
		if err := s.store.Delete(ctx, claim.ObjectKey); err != nil {
			result.Failed++
			_ = s.repository.ReleaseImageCleanup(ctx, claim)
			continue
		}
		if err := s.repository.FinishImageCleanup(ctx, claim); err != nil {
			result.Failed++
			_ = s.repository.ReleaseImageCleanup(ctx, claim)
			continue
		}
