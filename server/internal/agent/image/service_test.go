...
func TestUploadStoresValidatedImageAndCommitsFencedUpload(t *testing.T) {
	...
	if asset.ID != testAssetID ||
		asset.ObjectKey != core.AgentImageObjectPrefix+testAssetID+".png" ||
...
func TestUploadTelemetryIsUsefulAndDoesNotLeakObjectData(t *testing.T) {
	...
	for _, secret := range []string{
		core.AgentImageObjectPrefix + testAssetID + ".png",
		"signature=",
		"checksum_sha256",
	} {
...
func (r *imageTestRepository) StageImageAsset(
	_ context.Context,
	candidate Asset,
) (core.ImageAssetStage, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.stageCalls++
	if r.asset.ID == "" {
		r.asset = candidate
		return core.ImageAssetStage{Asset: r.asset, Created: true}, nil
	}
	return core.ImageAssetStage{Asset: r.asset, Created: false}, nil
}

func (r *imageTestRepository) ClaimImageUpload(
	_ context.Context,
	ownerID string,
	assetID string,
	lease time.Duration,
) (core.ImageUploadClaim, bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.asset.OwnerID != ownerID || r.asset.ID != assetID {
		return core.ImageUploadClaim{}, false, ErrNotFound
	}
	if r.asset.ETag != "" {
		return core.ImageUploadClaim{Asset: r.asset}, false, nil
	}
	r.asset.UploadFencingToken++
	r.asset.UploadLeaseUntil = time.Now().UTC().Add(lease)
	return core.ImageUploadClaim{
		Asset:          r.asset,
...
func (r *imageTestRepository) CommitImageUpload(
	_ context.Context,
	ownerID string,
	assetID string,
	token uint64,
...
func (r *imageTestRepository) FindImageAsset(
	_ context.Context,
	ownerID string,
	assetID string,
) (Asset, error) {
...
func (r *imageTestRepository) ClaimImageCleanup(
	_ context.Context,
	_ time.Duration,
	_ int,
) ([]CleanupClaim, error) {
...
func (r *imageTestRepository) FinishImageCleanup(
	_ context.Context,
	_ CleanupClaim,
) error {
	r.mu.Lock()
...
func (r *imageTestRepository) ReleaseImageCleanup(
	context.Context,
	CleanupClaim,
) error {
	return nil
