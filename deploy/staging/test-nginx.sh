...
mkdir -p "$temporary_directory/acme" \
  "$temporary_directory/candidate-public/downloads/android/candidates/123456789" \
...
candidate_directory="$temporary_directory/candidate-public/downloads/android/candidates/123456789"
printf '%s\n' 'signed-staging-apk-fixture' > \
  "$candidate_directory/speakup-v0.1.0-staging-arm64.apk"
apk_sha=$(sha256sum \
  "$candidate_directory/speakup-v0.1.0-staging-arm64.apk" |
  awk '{print $1}')
apk_size=$(stat -c '%s' \
  "$candidate_directory/speakup-v0.1.0-staging-arm64.apk" 2>/dev/null ||
  stat -f '%z' \
    "$candidate_directory/speakup-v0.1.0-staging-arm64.apk")
printf '%s  %s\n' "$apk_sha" 'speakup-v0.1.0-staging-arm64.apk' > \
  "$candidate_directory/speakup-v0.1.0-staging-arm64.apk.sha256"
...
cp \
  "$candidate_directory/candidate.json" \
  "$temporary_directory/candidate-public/downloads/android/staging-candidate.json"
openssl req \
  -x509 \
  -newkey rsa:2048 \
  -nodes \
...
assert_contains 'root /var/lib/speakup/staging-apk-public;'
...
if grep -Fq 'root /var/lib/speakup/staging-apk-public;' "$api_configuration"; then
...
docker run --rm \
  ...
  --volume "$temporary_directory/candidate-public:/var/lib/speakup/staging-apk-public:ro" \
...
unauthenticated_status=$(curl \
  ...
  --write-out '%{http_code}' \
  --resolve "staging.speak-up.top:$https_port:127.0.0.1" \
  "https://staging.speak-up.top:$https_port/downloads/android/staging-candidate.json")
...
assert_curl_succeeds 'authenticated current candidate metadata request' \
  --insecure \
  --silent \
...
assert_curl_succeeds 'authenticated immutable candidate metadata request' \
  --insecure \
  --silent \
  --user 'staging:test-password' \
  ...
  --output /dev/null \
  --resolve "staging.speak-up.top:$https_port:127.0.0.1" \
  "https://staging.speak-up.top:$https_port/downloads/android/candidates/123456789/candidate.json"
...
assert_curl_succeeds 'authenticated checksum download request' \
  ...
  --resolve "staging.speak-up.top:$https_port:127.0.0.1" \
  "https://staging.speak-up.top:$https_port/downloads/android/candidates/123456789/speakup-v0.1.0-staging-arm64.apk.sha256"
cmp \
  "$checksum_body" \
  "$candidate_directory/speakup-v0.1.0-staging-arm64.apk.sha256"
...
assert_curl_succeeds 'authenticated APK HEAD request' \
  ...
  --output /dev/null \
  --resolve "staging.speak-up.top:$https_port:127.0.0.1" \
  "https://staging.speak-up.top:$https_port/downloads/android/candidates/123456789/speakup-v0.1.0-staging-arm64.apk"
...
for path in \
  ...
  /downloads/android/candidates/0/candidate.json \
  /downloads/android/candidates/123456789/unknown.txt \
  /downloads/android/candidates/123456789/speakup-v0.1.0-production-arm64.apk; do
  status=$(curl \
    --insecure \
    --silent \
    --user 'staging:test-password' \
...
api_status=$(curl \
  ...
  "https://staging-api.speak-up.top:$https_port/downloads/android/staging-candidate.json")
[[ "$api_status" == 404 ]] || {
